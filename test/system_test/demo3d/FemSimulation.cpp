...
float Lame_parameter_2 =
    Young_E * Poisson_r / ((1 + Poisson_r) * (1 - 2 * Poisson_r));
...
void FemSimulation::compute_total_energy() {
  ...
  for (int i = 0; i < n_fem_element; ++i) {
    ...
    float element_energy_density = 0.5 * Lame_parameter_1 * (I1 - dim) -
                                   Lame_parameter_1 * log(J) +
                                   0.5 * Lame_parameter_2 * log(J) * log(J);
...
void FemSimulation::InitShell() {
  ...
  for (int i = 0; i < n_node_y - scale; i += scale) {
    for (int j = 0; j < n_node_z - scale; j += scale) {
      mesh_index_list.push_back({mesh(0, i, j), mesh(0, i + scale, j),
                                 mesh(0, i + scale, j + scale),
                                 mesh(0, i, j + scale)});
      mesh_index_list.push_back({mesh(n_node_x - scale, i, j),
                                 mesh(n_node_x - scale, i, j + scale),
                                 mesh(n_node_x - scale, i + scale, j + scale),
                                 mesh(n_node_x - scale, i + scale, j)});
    }
  }
  ...
  for (int i = 0; i < n_node_z - scale; i++) {
    for (int j = 0; j < n_node_x - scale; j++) {
      mesh_index_list.push_back({mesh(j, 0, i), mesh(j, 0, i + scale),
                                 mesh(j + scale, 0, i + scale),
                                 mesh(j + scale, 0, i)});
      mesh_index_list.push_back({mesh(j, n_node_y - scale, i),
                                 mesh(j + scale, n_node_y - scale, i),
                                 mesh(j + scale, n_node_y - scale, i + scale),
                                 mesh(j, n_node_y - scale, i + scale)});
    }
  }
  ...
  for (int i = 0; i < n_node_x - scale; i++) {
    for (int j = 0; j < n_node_y - scale; j++) {
      mesh_index_list.push_back({mesh(i, j, 0), mesh(i + scale, j, 0),
                                 mesh(i + scale, j + scale, 0),
                                 mesh(i, j + scale, 0)});
      mesh_index_list.push_back({mesh(i, j, n_node_z - scale),
                                 mesh(i, j + scale, n_node_z - scale),
                                 mesh(i + scale, j + scale, n_node_z - scale),
                                 mesh(i + scale, j, n_node_z - scale)});
    }
  }
}
  ...
  for (int i = 0; i < n_node_x; ++i) {
    for (int j = 0; j < n_node_y; ++j) {
      for (int k = 0; k < n_node_z; ++k) {
        ...
        this->points[idx][0] =
            -(deltax * 0.5f * n_node_x) / 2 + sphere.x + i * deltax * 0.5f;
        this->points[idx][1] = 0.5f + j * deltax * 0.5f + i * deltax * 0.1f;
        this->points[idx][2] =
            -(deltax * 0.5f * n_node_z) / 2 + sphere.z + k * deltax * 0.5f;
  ...
  for (int i = 0; i < n_node_x - 1; ++i) {
    for (int j = 0; j < n_node_y - 1; ++j) {
      for (int k = 0; k < n_node_z - 1; ++k) {
        // element id
        int eidx =
            (k * (n_node_y - 1) * (n_node_x - 1) + j * (n_node_x - 1) + i) * 4;
  ...
  if (this->State == SCENE_ACTIVE) {
    ...
    for (int i = 0; i < iterate_time; ++i) {
      ...
      for (int i = 0; i < n_node; ++i) {
        ...
        // Vector2f diffUtoX = Vector2f(abs(deltaX[0]) > 1e-4 ? deltaU /
        // deltaX[0] : 0.0f, abs(deltaX[1]) > 1e-4 ? deltaU / deltaX[1] : 0.0f);

        // diffUtoX = Vector2f(0.0f,0.0f);

        // cout << diffUtoX << endl;

        // this->points_v[i] = (this->points_v[i] + ((- diffUtoX / node_mass) +
        // Vector2f(0.0f, -10.0f)) * deltat) * exp(deltat * -6);

        this->points_v[i] =
            (this->points_v[i] + (this->points_force[i] / node_mass) * deltat) *
            exp(deltat * -3);
