...
inline int mesh(int i, int j) { return i * n_node_y + j; }
...
dtkFemSimulation::dtkFemSimulation(unsigned int width, unsigned int height)
    : dtkScene(width, height), points(n_node), pre_points(n_node),
      ...
      MeshTable(n_fem_element, std::vector<int>(3, 0)),
  ...
  if (this->State == SCENE_ACTIVE) {
    ...
    for (int i = 0; i < iterate_time; ++i) {
      ...
      for (int j = 0; j < n_node; ++j) {
        ...
        this->points_v[j] =
            (this->points_v[j] + (this->points_force[j] / node_mass) * deltat) *
            exp(deltat * -3);
...
void dtkFemSimulation::Render() {
  ...
  int n = 100;
  for (int i = 0; i < n; i++) {
    glVertex2f(center[0] + radius * cos(2 * dtk::dtkPI / n * i),
               center[1] + radius * sin(2 * dtk::dtkPI / n * i));
  }
  glEnd();
