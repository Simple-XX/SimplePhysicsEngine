...
const cworld::pair_list &cworld::get_arbiters() const { return _arbiters; }
...
bool cworld::is_pause() const { return _pause; }

void cworld::set_pause(bool pause) { _pause = pause; }
...
polygon_body::ptr cfactory::make_box(decimal mass, decimal width,
                                     decimal height, const vec2 &position) {
  // 四边形
  // 注意顶点呈逆时针
  polygon_body::vertex_list vertices = {{width / 2, height / 2},
                                        {-width / 2, height / 2},
                                        {-width / 2, -height / 2},
                                        {width / 2, -height / 2}};
...
polygon_body::ptr
cfactory::make_polygon(decimal mass, const polygon_body::vertex_list &vertices,
                       const vec2 &position) {
  auto body = std::make_shared<polygon_body>(global_id++, mass, vertices);
  body->set_position(position);
  return body;
...
cpair::ptr cfactory::make_arbiter(cbody::ptr a, cbody::ptr b,
                                  const vec2 &normal,
                                  const cpair::contact_list &contacts) {
  return std::make_shared<cpair>(a, b, normal, contacts);
}

revolute_joint::ptr cfactory::make_revolute_joint(cbody::ptr a, cbody::ptr b,
                                                  const vec2 &anchor) {
