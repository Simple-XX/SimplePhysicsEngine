...
uint16_t cbody::get_id() const { return _id; }

decimal cbody::get_mass() const { return _mass; }
...
decimal cbody::get_inv_mass() const { return _inv_mass; }

decimal cbody::get_inertia() const { return _inertia; }
...
decimal cbody::get_inv_inertia() const { return _inv_inertia; }

const vec2 &cbody::get_centroid() const { return _centroid; }

void cbody::set_centroid(const vec2 &centroid) { _centroid = centroid; }

const vec2 &cbody::get_position() const { return _position; }

void cbody::set_position(const vec2 &position) { _position = position; }

const mat22 &cbody::get_rotation() const { return _rotation; }

void cbody::set_rotation(const mat22 &rotation) { _rotation = rotation; }

const vec2 &cbody::get_velocity() const { return _velocity; }

void cbody::set_velocity(const vec2 &velocity) { _velocity = velocity; }

decimal cbody::get_angular_velocity() const { return _angular_velocity; }

void cbody::set_angular_velocity(decimal angular_velocity) {
  _angular_velocity = angular_velocity;
}

const vec2 &cbody::get_force() const { return _force; }

void cbody::set_force(const vec2 &force) { _force = force; }

decimal cbody::get_torque() const { return _torque; }

void cbody::set_torque(decimal torque) { _torque = torque; }

decimal cbody::get_friction() const { return _friction; }

void cbody::set_friction(decimal friction) { _friction = friction; }
...
// 计算多边形转动惯量
static decimal calc_polygon_inertia(decimal mass,
                                    const cbody::vertex_list &vertices) {
...
polygon_body::polygon_body(uint16_t id, decimal mass,
                           const cbody::vertex_list &vertices)
...
// 轴：只需采用两个凸包的每个条做检测即可
decimal polygon_body::min_separating_axis(size_t &idx,
                                          const polygon_body &other) const {
