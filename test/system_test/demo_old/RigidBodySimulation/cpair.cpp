...
cpair::cpair(cbody::ptr a, cbody::ptr b, const vec2 &normal,
             const cpair::contact_list &contacts)
    : _a(a), _b(b), _normal(normal), _contacts(contacts) {}

const cpair::contact_list &cpair::get_contacts() const { return _contacts; }

const vec2 &cpair::get_normal() const { return _normal; }
...
void cpair::pre_step(decimal dt) {
  ...
  for (auto &contact : _contacts) {
    auto kn = a->get_inv_mass() + b->get_inv_mass() +
              dot(a->get_inv_inertia() *
                          cross(cross(contact.ra, _normal), contact.ra) +
                      b->get_inv_inertia() *
                          cross(cross(contact.rb, _normal), contact.rb),
                  _normal);
    auto kt = a->get_inv_mass() + b->get_inv_mass() +
              dot(a->get_inv_inertia() *
                          cross(cross(contact.ra, tangent), contact.ra) +
                      b->get_inv_inertia() *
                          cross(cross(contact.rb, tangent), contact.rb),
                  tangent);

    contact.mass_normal = 1 / kn;
    contact.mass_tangent = 1 / kt;
    contact.bias = -kBiasFactor / dt *
                   std::min(0.0, contact.separation + kAllowedPenetration);
  }
}
...
void cpair::update_impulse() {
  ...
  for (auto &contact : _contacts) {
    vec2 dv =
        (b->get_velocity() + cross(b->get_angular_velocity(), contact.rb)) -
    ...
    dpt = std::max(-friction * contact.pn,
                   std::min(friction * contact.pn, contact.pt + dpt)) -
          contact.pt;
...
void cpair::update(const cpair &old_arbiter) {
  ...
  for (auto &new_contact : _contacts) {
    auto old_contact =
        std::find(old_contacts.begin(), old_contacts.end(), new_contact);
...
static size_t clip(cpair::contact_list &contacts_out,
                   const cpair::contact_list &contacts_in, size_t idx,
...
cpair::ptr cpair::is_collide(polygon_body::ptr &pa, polygon_body::ptr &pb,
                             uint32_t &id) {
  auto _pa = &pa;
  auto _pb = &pb;
  size_t ia, ib;
  ...
  cpair::contact_list contacts = {{b, idx}, {b, next_idx}};
