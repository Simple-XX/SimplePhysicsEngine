...
decimal vec2::magnitude() const { return std::sqrt(x * x + y * y); }

vec2 vec2::normal() const { return vec2(y, -x).normalized(); }

vec2 vec2::normalized() const { return *this / magnitude(); }

vec2 operator+(const vec2 &a, const vec2 &b) { return {a.x + b.x, a.y + b.y}; }

void operator+=(vec2 &a, const vec2 &b) { a = a + b; }

vec2 operator-(const vec2 &a, const vec2 &b) { return {a.x - b.x, a.y - b.y}; }

void operator-=(vec2 &a, const vec2 &b) { a = a - b; }

vec2 operator*(const vec2 &a, decimal b) { return {a.x * b, a.y * b}; }

vec2 operator*(decimal a, const vec2 &b) { return b * a; }

void operator*=(vec2 &a, decimal b) { a = a * b; }
...
void operator+=(mat22 &a, const mat22 &b) { a = a + b; }

mat22 operator+(const mat22 &a, decimal b) { return a + b * mat22::I; }

mat22 operator+(decimal a, const mat22 &b) { return b + a; }

void operator+=(mat22 &a, decimal b) { a = a + b; }
...
mat22 operator-(decimal a, const mat22 &b) { return a * mat22::I - b; }

void operator-=(mat22 &a, decimal b) { a = a - b; }

mat22 operator*(const mat22 &a, decimal b) {
  return {a[0].x * b, a[0].y * b, a[1].x * b, a[1].y * b};
}
...
void operator*=(vec2 &a, const mat22 &b) { a = a * b; }
...
vec2 cross(decimal a, const vec2 &b) { return a * vec2(-b.y, b.x); }

decimal cross(const vec2 &a, const vec2 &b) { return a.x * b.y - a.y * b.x; }

decimal dot(const vec2 &a, const vec2 &b) { return a.x * b.x + a.y * b.y; }
