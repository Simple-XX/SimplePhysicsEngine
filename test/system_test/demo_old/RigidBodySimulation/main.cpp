...
static void test_polygon() {
  ...
  world.add(
      clib::cfactory::make_polygon(200, {{-1, 0}, {1, 0}, {0, 1}}, {-1, 0}));
  world.add(
      clib::cfactory::make_polygon(200, {{-1, 0}, {1, 0}, {0, 1}}, {1, 0}));
...
static void test_joint() {
  ...
  for (size_t i = 0; i < 5; ++i) {
    ...
    world.add(box2);
    auto joint2 =
        clib::cfactory::make_revolute_joint(ground, box2, {3.5f - i, 11});
    world.add(joint2);
  }
}
...
static void test_chain() {
  ...
  for (int i = 0; i < 15; ++i) {
    ...
    world.add(box);
    auto joint =
        clib::cfactory::make_revolute_joint(last, box, clib::vec2(i, y));
    world.add(joint);
    last = box;
  }
}

void init() { test_joint(); }
...
void move(const clib::vec2 &v) { world.move(v); }
