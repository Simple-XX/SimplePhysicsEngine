...
cbody::ptr cjoint::get_a() const { return _a.lock(); }

void cjoint::set_a(cbody::ptr a) { _a = a; }

cbody::ptr cjoint::get_b() const { return _b.lock(); }

void cjoint::set_b(cbody::ptr b) { _b = b; }
...
revolute_joint::revolute_joint(cbody::ptr a, cbody::ptr b, const vec2 &anchor)
  ...
  _local_anchor_a = a->get_rotation().transpose() *
                    (_anchor - a->local_to_world(a->get_centroid()));
  _local_anchor_b = b->get_rotation().transpose() *
                    (_anchor - b->local_to_world(b->get_centroid()));
}
...
void revolute_joint::pre_step(decimal dt) {
  ...
  auto k = (a->get_inv_mass() + b->get_inv_mass()) * mat22::I +
           a->get_inv_inertia() * mat22(_ra.y * _ra.y, -_ra.y * _ra.x,
                                        -_ra.y * _ra.x, _ra.x * _ra.x) +
           b->get_inv_inertia() * mat22(_rb.y * _rb.y, -_rb.y * _rb.x,
                                        -_rb.y * _rb.x, _rb.x * _rb.x);
  _mass = k.inverse();
  _bias = -kBiasFactor / dt *
          (b->local_to_world(b->get_centroid()) + _rb -
           a->local_to_world(a->get_centroid()) - _ra);
...
vec2 revolute_joint::world_anchor_a() const {
  auto a = _a.lock();
  return a->local_to_world(a->get_rotation() * _local_anchor_a +
                           a->get_centroid());
}

vec2 revolute_joint::world_anchor_b() const {
  auto b = _b.lock();
  return b->local_to_world(b->get_rotation() * _local_anchor_b +
                           b->get_centroid());
}
} // namespace clib
