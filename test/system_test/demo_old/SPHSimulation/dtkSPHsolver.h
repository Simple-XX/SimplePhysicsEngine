 ...
 *
 */
enum SPHMethod { WCSPH = 0, PCISPH = 1, DFSPH = 2 };
...
private:
  ...
  vector<VectorXf>
      particle_position_new; /**< Prediction position values for PCISPH */
  vector<VectorXf>
      particle_velocity_new; /**< Prediction velocity values for PCISPH */
  ...
  unordered_map<int, int>
      grid_num_particles; /**< particles num of every grid */
  unordered_map<int, vector<int>>
      grid2particles; /**<every grid cotains particle index */
  vector<int>
      particle_num_neigbors; /** neighbor particles num of every paritcle */
  vector<vector<int>>
      particle_neighbors; /**< every particles's neighbor particle index */
  ...
  bool is_in_grid(int cell) { bool res = true; }

public:
  dtkSPHSolver(const vector<int> &res, float screen_to_world_ratio,
               const vector<int> &bound, float alpha, float dx,
               int max_num_paticles, bool dynamic_allocate,
               bool adaptive_time_step, enmu SPHMethod method);
