...
dtkSPHSolver::dtkSPHSolver(const vector<int> &res, float screen_to_world_ratio,
                           const vector<int> &bound, float alpha = 0.5,
                           float dx = 0.2 int max_num_paticles = (1 << 20),
                           bool dynamic_allocate = false,
                           bool adaptive_time_step = true,
                           enmu SPHMethod method = SPHMethod::WCSPH)
    : res(res), screen_to_world_ratio(screen_to_world_ratio), alpha(alpha),
      dx(dx), max_num_paticles(max_num_paticles),
  ...
  grid_size = 2 * dh;
  grid_pos = Vector2i(ceil(res[0] / screen_to_world_ratio / grid_size),
                      ceil(res[1] / screen_to_world_ratio / grid_size));
