...
guideWireInternalResponse::guideWireInternalResponse(
    double radius, double lineDensity, double yougModulus, double shearModulus,
    double stretchModulus, double springConst, double segInterval,
    double length, double defaultDamp, double defaultPointDamp,
    double defaultPointResistence, dtkT3<double> defaultGravityAccel,
...
bool guideWireInternalResponse::Update(double timeslice, ItrMethod method,
  ...
  case Collision:
    ...
    /*dynamicSuit::Update(mMassPoints, mSegInterval, splitAngle, mergeAngle,
     * maxSegLength, minSegLength, mDefaultMass, mDefaultPointDamp,
     * mDefaultPointResistence, mDefaultGravityAccel );*/
    ResistStretch(timeslice);

    for (unsigned int i = 0; i < mMassPoints.size() - 1; i++) {
      double length = GK::Length(mMassPoints[i]->GetPoint() -
                                 mMassPoints[i + 1]->GetPoint());
      if (length < 0.001)
    ...
    //{
    //	double length = GK::Length(mMassPoints[i]->GetPoint() -
    //mMassPoints[i+1]->GetPoint()); 	if (length < 0.001) 		std::cout << "length is
    //less than 0.1" << std::endl;
    //
    // }
...
bool guideWireInternalResponse::UpdateMassPoints(double timeslice,
                                                 ItrMethod method,
                                                 dtkID iteration) {
...
void guideWireInternalResponse::ResistStretch(double timeslice) {
  ...
  while (count < constraintTimes) {
    ...
    for (dtkID i = 0; i < size - 1; i++) {
      ...
      // mMassPoints[i]->AddForce(vector3TodtkT3(eachDifferPosition[i]) /
      // timeslice / timeslice * mMassPoints[i]->GetMass() );
  ...
  for (dtkID i = 0; i < size - 1; i++) {
    ...
    // mMassPoints[i]->AddForce(vector3TodtkT3(sumDifferPosition[i]) / timeslice
    // / timeslice * mMassPoints[i]->GetMass());
  }
}

void guideWireInternalResponse::ResistOverBend(double bendAngle,
                                               double timeslice) {
  ...
  while (count < constraintTimes) {
    ...
    for (dtkID id = 1; id < size - 1; id++) {
      ...
      if (constranitValue < 0) {
        ...
        double lamda =
            constranitValue /
            (pow(rightEdgeLength, 2) +
             GK::DotProduct(twoEdgeDiff, twoEdgeDiff) + pow(leftEdgeLength, 2));
    ...
    for (dtkID i = 0; i < size; i++) {
      mMassPoints[i]->SetPoint(
          mMassPoints[i]->GetPoint() + eachDifferPosition[i], false);
      // mMassPoints[i]->AddForce(vector3TodtkT3(eachDifferPosition[i] ) *(
      // mMassPoints[i]->GetMass() / 100.0 / timeslice / timeslice ));
    }
  }
...
void guideWireInternalResponse::AddBendForce(double timeslice) {
  ...
  //	kb = crossProd / (0.5 * (mSegInterval[id-1] * mSegInterval[id] +
  //dotProd));

  //	if (GK::DotProduct(kb, kb) < 0.0000001)
  //		continue;

  //	bendForce1 = - 2 * GK::CrossProduct(rightEdge, kb) +  rightEdge *
  //GK::DotProduct(kb, kb);                                       // left
  //	bendForce2 = 2 * GK::CrossProduct(leftEdge + rightEdge, kb) - (rightEdge
  //- leftEdge)*GK::DotProduct(kb, kb);      // middle 	bendForce3 = -(bendForce1
  //+ bendForce2);
  //// right

  //	double coefficient = (-2*mBendModulus / (mSegInterval[id-1] +
  //mSegInterval[id]) ) / (mSegInterval[id-1]*mSegInterval[id] + dotProd) ;
  ...
  for (dtkID i = 0; i <= mMassPoints.size() - 3; i++)
  {
          ...
          theta = acos(GK::DotProduct(rightEdge1, rightEdge2) /
  (lengthRightEdge1 * lengthRightEdge2) );
          ...
          {
                  bendForceDirection =
  GK::Normalize(GK::CrossProduct(GK::CrossProduct(rightEdge1, rightEdge2),
  rightEdge1) ); bendForce = vector3TodtkT3(mBendModulus * theta *
  bendForceDirection); mMassPoints[i]->AddForce(bendForce,false);
          }
  }*/
  ...
  for (dtkID id = 1; id < mMassPoints.size() - 1; id++) {
    ...
    bendForce1 = (-mBendModulus * theta) *
                 GK::Normalize(GK::CrossProduct(crossProd, leftEdge)); // left
    bendForce3 = (-mBendModulus * theta) *
                 GK::Normalize(GK::CrossProduct(crossProd, rightEdge)); // right
...
void guideWireInternalResponse::AddTwistForce(double timeslice) {
  ...
  for (dtkID id = 1; id < mMassPoints.size() - 1; id++) {
    ...
    crossProd = GK::CrossProduct(leftEdge, rightEdge);
    kb =
        crossProd / (0.5 * (mSegInterval[id - 1] * mSegInterval[id] + dotProd));
    ...
    double coefficient =
        mTwistModulus * mTwist /
        ((mSegInterval[id - 1] + mSegInterval[id]) * mPts->GetNumberOfPoints());
  ...
  // TwistForce = mTwistModulus * mTwistInterval / (mSegInterval *
  // mPts->GetNumberOfPoints()) * TwistForce;
  ...
  dtkT3<double> dampForce = -dir * ( dot( v, dir ) * (timeslice * mTwistModulus
  * mDefaultMass * TwistLength * 0.5 ) );
  */
...
// encapsulated impulse function, as to transparency impulse because the guide
// wire inextensible
void guideWireInternalResponse::AddImpulse(dtkID id,
                                           const dtkT3<double> &impulse) {
  // transmit to left
  ...
  while (tempLeftID > 0) {
    ...
    if (leftDot <= 0.0001 && leftDot >= -0.0001) {
      ...
      // transmit, continue
      leftImpulse = dot(leftImpulse, leftEdgeT3) / dot(leftEdgeT3, leftEdgeT3) *
                    leftEdgeT3;
  ...
  while (tempRightID + 1 < mPts->GetMaxID()) {
    ...
    if (rightDot <= 0.0001 && rightDot >= -0.0001) {
      ...
      // transmit, continue
      rightImpulse = dot(rightImpulse, rightEdgeT3) /
...
// encapsulated force function, as to transparency force because the guide wire
// inextensible
void guideWireInternalResponse::AddForce(dtkID id, const dtkT3<double> &force) {
  ...
  while (tempLeftID > 0) {
    ...
    if (leftDot <= 0.0001 && leftDot >= -0.0001) {
      ...
      // transmit, continue
      leftForce =
          dot(leftForce, leftEdgeT3) / dot(leftEdgeT3, leftEdgeT3) * leftEdgeT3;
...
void guideWireInternalResponse::AddPosition(dtkID id,
                                            const GK::Vector3 &positionChange) {
  // transmit to left
  GK::Vector3 leftEdge, rightEdge;
  ...
  while (tempLeftID > 0) {
    ...
    if (leftDot <= 0.0001 && leftDot >= -0.0001) {
      ...
      // transmit, continue
      leftPositionChange = GK::DotProduct(leftPositionChange, leftEdge) /
                           GK::DotProduct(leftEdge, leftEdge) * leftEdge;
      mMassPoints[tempLeftID - 1]->SetPoint(
          mMassPoints[tempLeftID - 1]->GetPoint() + leftPositionChange);
    }
  ...
  while (tempRightID + 1 < mPts->GetMaxID()) {
    ...
    if (rightDot <= 0.0001 && rightDot >= -0.0001) {
      ...
      // transmit, continue
      rightPositionChange = GK::DotProduct(rightPositionChange, rightEdge) /
                            GK::DotProduct(rightEdge, rightEdge) * rightEdge;
      mMassPoints[tempRightID + 1]->SetPoint(
          mMassPoints[tempRightID + 1]->GetPoint() + rightPositionChange);
    }
...
void guideWireInternalResponse::ConstructGuideWire(
    dtkPoints::Ptr points, const std::vector<double> &segInterval) {
...
dtkPoints::Ptr guideWireInternalResponse::GetPoints() { return mPts; }
...
dtkID guideWireInternalResponse::AddMassPoint(
    dtkID id, const double &mass, const dtkT3<double> &vel, double pointDamp,
    double pointResistence, dtkT3<double> defaultGravityAccel) {
...
void guideWireInternalResponse::ApplyExternalVelImpulse(
    const dtkT3<double> &velImpulse) {
  // AddImpulse(mMassPoints.size() - 1, velImpulse);
  if (velImpulse.y > 0 && velImpulse.x == 0) {
    double velImpulseLength = length(velImpulse);
    dtkT3<double> edge =
        vector3TodtkT3(mMassPoints[0]->GetPoint() - mMassPoints[1]->GetPoint());
    ...
    dtkT3<double> verticalEdgeImpulse =
        velImpulseLength * cross(crossProd, edge);
    ...
    double velImpulseLength = length(velImpulse);
    dtkT3<double> edge =
        vector3TodtkT3(mMassPoints[mMassPoints.size() - 1]->GetPoint() -
                       mMassPoints[mMassPoints.size() - 2]->GetPoint());
    dtkT3<double> verticalEdgeImpulse = velImpulseLength * normalize(edge);
...
void guideWireInternalResponse::RotateMassPoint(dtkPoints::Ptr points,
                                                dtkID rotatePointID,
                                                dtkID axisPointID1,
                                                dtkID axisPointID2,
                                                double angle) {
