...
public:
  ...
  static Ptr
  New(double radius = 0.001, double lineDensity = 7860,
      double yougModulus = 200, double shearModulus = 100,
      double stretchModulus = 100, double springConst = 300,
      double segInterval = 0.1, double length = 0.1, double defaultDamp = 5880,
      double defaultPointDamp = 0.8, double defaultPointResistence = 0.0,
      dtkT3<double> defaultGravityAccel = dtkT3<double>(0.0, 0.0, 0.0),
      double defaultCurvature = 0.0) {
    return Ptr(new guideWireInternalResponse(
        radius, lineDensity, yougModulus, shearModulus, stretchModulus,
        springConst, segInterval, length, defaultDamp, defaultPointDamp,
        defaultPointResistence, defaultGravityAccel, defaultCurvature));
  }
  ...
  void ConstructGuideWire(dtkPoints::Ptr points,
                          const std::vector<double> &segInterval);
  ...
  dtkID AddMassPoint(dtkID id, const double &mass = 1.0,
                     ...
                     dtkT3<double> defaultGravityAccel = dtkT3<double>(0, 0,
                                                                       0));
...
private:
  guideWireInternalResponse(double radius, double lineDensity,
                            double yougModulus, double shearModulus,
                            double stretchModulus, double springConst,
                            double segInterval, double length,
                            double defaultDamp, double defaultPointDamp,
                            double defaultPointResistence,
                            dtkT3<double> defaultGravityAccel,
