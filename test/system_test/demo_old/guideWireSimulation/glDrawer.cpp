...
void GLDrawer::paintGL() {
  ...
  GLfloat lightPositionVascular[4] = {
      (eyePosition.x - centerPosition.x) * xscale,
      (eyePosition.y - centerPosition.y) * yscale,
      (eyePosition.z - centerPosition.z) * zscale, 0};
  GLfloat lightPositionGuideWire[4] = {
      -(eyePosition.x - centerPosition.x) * xscale,
      -(eyePosition.y - centerPosition.y) * yscale,
      -(eyePosition.z - centerPosition.z) * zscale, 0};
  ...
  // draw the scene:
  glPushMatrix();
  gluLookAt(eyePosition.x + 5 * (eyePosition.x - centerPosition.x),
            eyePosition.y + 5 * (eyePosition.y - centerPosition.y),
            eyePosition.z + 5 * (eyePosition.z - centerPosition.z),
            centerPosition.x, centerPosition.y, centerPosition.z, upVector.x,
            upVector.y, upVector.z);

  glTranslatef(xTranslate * screenXAxis.x, xTranslate * screenXAxis.y,
               xTranslate * screenXAxis.z);
  glTranslatef(yTranslate * upVector.x, yTranslate * upVector.y,
               yTranslate * upVector.z);
...
void GLDrawer::initializeVascularModel() {
  ...
  // ʹ�������幹�쵯���ʵ�
  core->CreateTetraMassSpring(vascularFile.c_str(), 0, 4, 7000, 80, 0.99, 0.2,
                              dtkT3<double>(0, 0, 0));
  ...
  // dtkStaticTriangleMeshWriter::Ptr triangleMeshWriter =
  // dtkStaticTriangleMeshWriter::New();
...
void GLDrawer::initializeGWModel() {
  ...
  for (dtkID i = 0; i < avoidPointSize; i++) {
    fileReadAvoidPoint >> avoidPoint[i]; // netgen���ɽ��
    avoidPoint[i] =
        avoidPoint[i] - 1; // because netgen pointID is bigger 1 more than .s3m
  }
  ...
  for (triangleID = 0; triangleID < avoid.size(); triangleID++) {
    if (avoid[triangleID].a == avoidPoint[0] ||
        avoid[triangleID].a == avoidPoint[0] ||
        avoid[triangleID] == avoidPoint[0])
      break;
  }

  triangleID = 0; // 78
  // ��������� ���ݽ���
  GK::Vector3 edge1 = points->GetPoint(avoid[triangleID].a) -
                      points->GetPoint(avoid[triangleID].b);
  GK::Vector3 edge2 = points->GetPoint(avoid[triangleID].b) -
                      points->GetPoint(avoid[triangleID].c);
  ...
  GK::Vector3 verticalVector =
      tipSegIntervalLength *
      GK::Normalize(GK::CrossProduct(normal, baseVector));
  // �����˵��᳤����
  ...
  for (dtkID i = tipOriginAngle.size(); i <= tipOriginAngle.size(); i--) {
    if (i == tipOriginAngle.size())
      guideWirePoints->SetPoint(
          i,
          guidewireStartPoint - tipAxisLength * tipSegIntervalLength * normal);
    else {
      int times = tipOriginAngle.size() - i;
      guideWirePoints->SetPoint(
          i,
          guideWirePoints->GetPoint(i + 1) +
              cos(times * bendAngleInterval) * tipSegIntervalLength * normal +
              sin(times * bendAngleInterval) * verticalVector);
    }
  }

  for (dtkID i = tipOriginAngle.size() + 1; i < guidewireSize; i++) {
    // ���쵼˿���岿
    guideWirePoints->SetPoint(i, guidewireStartPoint -
                                     (i - tipOriginAngle.size() +
                                      tipAxisLength * tipSegIntervalLength) *
                                         normal);
  }

  // ���쵼˿
  core->CreateGuideWire(1, guideWirePoints, tipOriginAngle.size(),
                        segIntervalLength, tipSegIntervalLength,
                        tipOriginAngle);
