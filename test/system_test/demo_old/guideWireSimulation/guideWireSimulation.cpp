...
guideWireSimulation::guideWireSimulation(QWidget *parent)
  ...
  this->setWindowTitle(QString("real-time simulate guide wire in minimally "
  ...
  connect(ui.actionSimulationParams, SIGNAL(triggered()), this,
          SLOT(simulationParams()));
  ...
  connect(simuParamsWidget.horizontalSlidergwStiff, SIGNAL(valueChanged(int)),
          simuParamsWidget.spinBoxgwStiff, SLOT(setValue(int)));
  connect(simuParamsWidget.horizontalSliderVascularStiff,
          SIGNAL(valueChanged(int)), simuParamsWidget.spinBoxVascularStiff,
          SLOT(setValue(int)));

  connect(simuParamsWidget.spinBoxgwStiff, SIGNAL(valueChanged(int)),
          simuParamsWidget.horizontalSlidergwStiff, SLOT(setValue(int)));
  connect(simuParamsWidget.spinBoxVascularStiff, SIGNAL(valueChanged(int)),
          simuParamsWidget.horizontalSliderVascularStiff, SLOT(setValue(int)));

  connect(simuParamsWidget.pushButtonApply, SIGNAL(pressed()), this,
          SLOT(apply()));
  connect(simuParamsWidget.pushButtonCancel, SIGNAL(pressed()), this,
          SLOT(cancel()));
...
void guideWireSimulation::idle() {
  // ��ȡPhantom Omni��״̬��Ϣ������Phantom Omni�ֱ����ٶ�
  // mDevicePhantomOmni->GetCurrentDisplayState();
  /*
  if (mCurrentDisplayState->button1)
  {
          // �����û�����˿������
          mPhantomOmniVelocity = (mCurrentDisplayState->position -
  mLastDisplayState->position) * 200.0;
          (OpenGLWidget->core)->ApplyExternalForce(1, dtkT3<double>(0,
  -mPhantomOmniVelocity[2], 0) );
  }
  else
  {
  ...
  */
  ...
  {
          mTimes ++;
          mPhantomOmniTwistVelocity += mCurrentDisplayState->gimbalAngle -
  mLastDisplayState->gimbalAngle; if (mTimes % 10 == 0)
          {
                  (OpenGLWidget->core)->ApplyExternalTwist(1,
  mPhantomOmniTwistVelocity[2]); mTimes = 0;
                  //mPhantomOmniTwistVelocity = hduVector3Dd(0, 0, 0);
          }
  }
*/
...
void guideWireSimulation::quit() {
  QMessageBox messagebox(QString("Quit Program?"),
                         QString("Do you real want to quit program?"),
                         QMessageBox::Information, QMessageBox::Yes,
                         QMessageBox::NoButton, QMessageBox::No, this);
  messagebox.show();
...
void guideWireSimulation::center() {
  ...
  if (OpenGLWidget->centerCount == 0) {
    ...
    dtkPoints::Ptr vascularPoints =
        (OpenGLWidget->core->mTriangleMeshes[0])->GetPoints();
    double xmin, xmax;
    double ymin, ymax;
    double zmin, zmax;
    ...
    if (indexAxis == 0) {
      ...
      OpenGLWidget->eyePosition =
          dtkT3<double>((xmin + xmax) / 2, ymin, (zmin + zmax) / 2);
      OpenGLWidget->centerPosition = dtkT3<double>(
          (OpenGLWidget->eyePosition).x, ymax, (OpenGLWidget->eyePosition).z);
      OpenGLWidget->screenXAxis = dtkT3<double>(0, 0, 1);

    } else if (indexAxis == 1) {
      OpenGLWidget->upVector = dtkT3<double>(0, 1, 0);
      OpenGLWidget->eyePosition =
          dtkT3<double>((xmin + xmax) / 2, (ymin + ymax) / 2, zmin);
      OpenGLWidget->centerPosition = dtkT3<double>(
          (OpenGLWidget->eyePosition).x, (OpenGLWidget->eyePosition).y, zmax);
      OpenGLWidget->screenXAxis = dtkT3<double>(1, 0, 0);

    } else {
      OpenGLWidget->upVector = dtkT3<double>(0, 0, 1);
      OpenGLWidget->eyePosition =
          dtkT3<double>(xmin, (ymin + ymax) / 2, (zmin + zmax) / 2);
      OpenGLWidget->centerPosition = dtkT3<double>(
          (xmax, OpenGLWidget->eyePosition).y, (OpenGLWidget->eyePosition).z);
...
void guideWireSimulation::about() {
  QString aboutString("This is real-time simulate guide wire in minimally "
                      "invasive vascular interventions \n ");
  QMessageBox messagebox(QString("About"), aboutString,
                         QMessageBox::Information, QMessageBox::NoButton,
...
void guideWireSimulation::howDoI() {
  ...
  operationGuide =
      operationGuide + QString("pull  guide wire :                      s\n");
  operationGuide =
      operationGuide + QString("left turn guide wire tip :           a\n");
  operationGuide =
      operationGuide + QString("right turn guide wire tip :         d\n");
  operationGuide =
      operationGuide + QString("translate up view frustum :      5\n");
  operationGuide =
      operationGuide + QString("translate down view frustum : 8\n");
  operationGuide =
      operationGuide + QString("translate left view frustum :     7\n");
  operationGuide =
      operationGuide + QString("translate right view frustum :   9\n");
  QMessageBox messagebox(QString("How Do I"), operationGuide,
                         QMessageBox::Information, QMessageBox::NoButton,
                         QMessageBox::NoButton, QMessageBox::NoButton, this);
  messagebox.exec();
}
