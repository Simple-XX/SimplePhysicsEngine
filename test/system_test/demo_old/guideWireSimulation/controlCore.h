...
public:
  ...
  double GetCollisionDetectTime() const { return mCollisionDetectTime; }
  double GetUpdateGuideWireTime() const { return mUpdateGuideWireTime; }
  double GetRotateOnceTime() const { return mRotateOnceTime; }
...
public:
  ...
  void CreateMassSpring(const char *filename, dtkID id, double point_mass,
                        ...
                        double pointResistence, dtkT3<double> gravityAccel,
  ...
  void CreateTetraMassSpring(const char *filename, dtkID id, double point_mass,
                             double stiffness, double damp, double pointDamp,
                             double pointResistence,
                             dtkT3<double> gravityAccel);
  void CreateTriangleSurfaceMesh(const char *filename, dtkID id);

  void CreateGuideWire(dtkID id, dtkPoints::Ptr points, dtkID lastTipID,
                       double segInterval, double tipSegInterval,
                       const std::vector<double> &tipOriginAngle);
  void UpdateGuideWireHierarchy(dtkID id, dtkPoints::Ptr points);

  void CreateCollisionResponse(dtkID object1_id, dtkID object2_id,
                               double strength);
  void UpdateCollisionResponse(dtkID object1_id, dtkID object2_id,
                               double strength);
...
private:
  vector<dtkID3> mAvoid; // Ѫ�ܵĿ��ڴ�����������Ƭ�Ķ���ID

  double mCollisionDetectTime; // the time cost of collision detection with one
                               // time step
  double mUpdateGuideWireTime; // the time cost of update the total guide wire
                               // with one time step
