...
void guideWireSimulation::idle() {
  ...
  if (mCurrentDisplayState->button1) {
    // �����û�����˿������
    mPhantomOmniVelocity =
        (mCurrentDisplayState->position - mLastDisplayState->position) * 200.0;
    (OpenGLWidget->core)
        ->ApplyExternalForce(1, dtkT3<double>(0, -mPhantomOmniVelocity[2], 0));
  } else {
  ...
  if (mCurrentDisplayState->button2) {
    mTimes++;
    mPhantomOmniTwistVelocity +=
        mCurrentDisplayState->gimbalAngle - mLastDisplayState->gimbalAngle;
    if (mTimes % 10 == 0) {
