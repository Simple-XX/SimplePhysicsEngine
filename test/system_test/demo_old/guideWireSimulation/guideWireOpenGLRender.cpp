...
void guideWireOpenGLRender::Draw() {
  ...
  for (j = 0; j < mGuideWirePoints->GetNumberOfPoints() - 1; j++) {
    RenderCylinder(qobj, mGuideWirePoints->GetPoint(j),
                   mGuideWirePoints->GetPoint(j + 1), mGuideWireRadius, 10, 10);
  }

  // glEnable(GL_DEPTH_TEST);
  for (j = 0; j < mGuideWirePoints->GetNumberOfPoints() - 1; j++) {
    RenderSphere(qobj, mGuideWirePoints->GetPoint(j), mGuideWireRadius * 0.96,
                 10, 10);
  }
...
void guideWireOpenGLRender::RenderCylinder(GLUquadricObj *qObj,
                                           const GK::Point3 &startCenterPoint,
                                           const GK::Point3 &endCenterPoint,
  ...
  GK::Vector3 rotateAxis =
      GK::CrossProduct(originCylinderDirection, cylinderDirection);
  double rotateAngle =
      acos(GK::DotProduct(cylinderDirection, originCylinderDirection) /
           GK::Length(cylinderDirection)) /
      pi * 180;
  GK::Point3 midCenterPoint = startCenterPoint + cylinderDirection / 2.0;

  // ����任˼·: ���Ƚ�����ת��Ȼ��ƽ�� (��������������ϵ)
  glPushMatrix();
  glTranslatef(startCenterPoint.x(), startCenterPoint.y(),
               startCenterPoint.z());
  glRotatef(rotateAngle, rotateAxis.x(), rotateAxis.y(), rotateAxis.z());
  gluCylinder(qObj, radius, radius, GK::Length(cylinderDirection), slices,
              stacks);
  glPopMatrix();
}

void guideWireOpenGLRender::RenderSphere(GLUquadricObj *qObj,
                                         const GK::Point3 &sphereCenter,
                                         double radius, int slices,
                                         int stacks) {
  glPushMatrix();
...
void guideWireOpenGLRender::computeBezPt(
    int nBeztPts, std::vector<dtkT3<double>> &bezPts, int nCtrlPts,
    const std::vector<dtkT3<double>> &ctrlPts) {
  ...
  for (int i = 0; i < nBeztPts; i++) {
    ...
    for (int j = 0; j < nCtrlPts; j++) {
      bezPts[i].x += Coeffs[j] * ctrlPts[j].x * pow(i * 1.0 / nBeztPts, j) *
                     pow(1 - i * 1.0 / nBeztPts, nCtrlPts - 1 - j);
      bezPts[i].y += Coeffs[j] * ctrlPts[j].y * pow(i * 1.0 / nBeztPts, j) *
                     pow(1 - i * 1.0 / nBeztPts, nCtrlPts - 1 - j);
      bezPts[i].z += Coeffs[j] * ctrlPts[j].z * pow(i * 1.0 / nBeztPts, j) *
                     pow(1 - i * 1.0 / nBeztPts, nCtrlPts - 1 - j);
    }
  }
}

void guideWireOpenGLRender::computeCtrlPt(
    double percent, const std::vector<dtkT3<double>> points,
  ...
  ctrlPts[size - 2][1] =
      points[size - 2] + percent * (points[size - 1] - points[size - 3]);
