...
// 找出最小间隙
static size_t incident_edge(const dtk::dtkDouble2 &N,
                            const dtk::dtkPolygonRigidBody &body) {
  size_t idx = SIZE_MAX;
...
static size_t nearest_edge(const dtk::dtkDouble2 &pos,
                           const dtk::dtkPolygonRigidBody &body) {
  size_t idx = SIZE_MAX;
  auto min_dot = dtk::inf;
  // 遍历B物体的边
...
void dtkCollisionPair::do_collision_mr(dtkMesh::ptr &pa,
  ...
  for (int i = 0; i < pa_cnt; i++) {
    ...
    bool within_edge =
        dot(va - vb, vb_next - vb) >= 0 && dot(va - vb_next, vb - vb_next) >= 0;
  ...
  for (int i = 0; i < pb_cnt; i++) {
    ...
    bool within_edge =
        dot(vb - va, va_next - va) >= 0 && dot(vb - va_next, va - va_next) >= 0;
