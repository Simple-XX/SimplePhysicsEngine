...
dtkMesh::dtkMesh(int id, int x_node, int y_node, Vector2f pos)
  ...
  Lame_parameter_2_ =
      Young_E_ * Poisson_r_ / ((1 + Poisson_r_) * (1 - 2 * Poisson_r_));
...
void dtkMesh::compute_total_energy() {
  ...
  for (int i = 0; i < n_fem_element_; ++i) {
    ...
    float element_energy_density = 0.5 * Lame_parameter_1_ * (I1 - dim_) -
                                   ...
                                   0.5 * Lame_parameter_2_ * log(J) * log(J);
...
void dtkMesh::updateShell() {
  ...
  for (int i = 1; i < n_node_x_; i++)
    shell_vertices.push_back(
        {this->points_[mesh(i, 0)][0], this->points_[mesh(i, 0)][1]});

  for (int i = 1; i < n_node_y_; i++)
    shell_vertices.push_back({this->points_[mesh(n_node_x_ - 1, i)][0],
                              this->points_[mesh(n_node_x_ - 1, i)][1]});

  for (int i = n_node_x_ - 2; i >= 0; i--)
    shell_vertices.push_back({this->points_[mesh(i, n_node_y_ - 1)][0],
                              this->points_[mesh(i, n_node_y_ - 1)][1]});

  for (int i = n_node_y_ - 2; i >= 0; i--)
    shell_vertices.push_back(
        {this->points_[mesh(0, i)][0], this->points_[mesh(0, i)][1]});
...
void dtkMesh::Init() {
  ...
  for (int i = 0; i < n_node_x_; ++i) {
    for (int j = 0; j < n_node_y_; ++j) {
      ...
      this->points_[idx][1] = position[1] +
                              (j - n_node_y_ / 2) * deltax * 0.5f +
                              (i - n_node_x_ / 2) * deltax * 0.1f;
