...
/**
 * @file dtkCollisionDetectHierarchy.cpp
 * @brief dtkCollisionDetectHierarchy 实现
...
void update(dtkID id, size_t numberOfThreads, barrier *enter_barrier,
...
#ifdef DTKCOLLISIONDETECTHIERARCHY_DEBUG
  cout << "[/dtkCollisionDetectHierarchy::~dtkCollisionDetectHierarchy]"
       << endl;
  cout << endl;
#endif
}
...
dtkCollisionDetectHierarchy::InsertTriangle(dtkPoints::Ptr pts, dtkID3 ids) {
  Primitive *primitive = new Primitive(dtkCollisionDetectPrimitive::TRIANGLE,
...
dtkCollisionDetectPrimitive *
dtkCollisionDetectHierarchy::InsertSegment(dtkPoints::Ptr pts, dtkID2 ids) {
  Primitive *primitive =
      new Primitive(dtkCollisionDetectPrimitive::SEGMENT, pts, ids[0], ids[1]);
  AddPrimitive(primitive);
  return primitive;
}

dtkCollisionDetectPrimitive *
dtkCollisionDetectHierarchy::InsertSphere(dtkPoints::Ptr pts, dtkID id) {
  Primitive *primitive =
      new Primitive(dtkCollisionDetectPrimitive::SPHERE, pts, id);
  AddPrimitive(primitive);
  return primitive;
}

void dtkCollisionDetectHierarchy::InsertTriangleMesh(
    dtkStaticTriangleMesh::Ptr triMesh, dtkID majorID, double extend) {
  triMesh->Rebuild();
...
void dtkCollisionDetectHierarchy::InsertTetraMesh(
    dtkStaticTetraMesh::Ptr tetraMesh, dtkID majorID, InsertOption opt,
    double extend) {
  tetraMesh->Rebuild();
  ...
  for (dtkID i = 0; i < ecTable.size(); i++) {
    ...
    for (dtkID j = 0; j < 4; j++) {
      tempOriFace =
          dtkID3(tetra[tetraMesh->EA2V[j][0]], tetra[tetraMesh->EA2V[j][1]],
                 tetra[tetraMesh->EA2V[j][2]]);
      tempSortedFace = tetraMesh->SortTriVertex(tempOriFace[0], tempOriFace[1],
                                                tempOriFace[2]);
...
void dtkCollisionDetectHierarchy::_UpdateAllPrimitives_mt() {
  ...
  parallel_for(blocked_range<size_t>(0, mPrimitives.size()),
               ApplyUpdate(&mPrimitives[0]), ap);
#else
  // boost thread
