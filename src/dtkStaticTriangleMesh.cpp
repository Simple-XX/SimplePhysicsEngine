...
/**
 * @file dtkStaticTriangleMesh.cpp
 * @brief dtkStaticTriangleMesh 实现
...
namespace dtk {
  ...
  dtkPoints::Ptr dtkStaticTriangleMesh::GetPoints() { return mPts; }
  ...
  void dtkStaticTriangleMesh::Clear() {
    ...
    mV2E.clear();
    if (mPts.get() != NULL)
      mV2E.resize(mPts->GetMaxID() + 1, dtkErrorID);
    mE2E.clear();
    mB2E.clear();
  }
  ...
  dtkID dtkStaticTriangleMesh::InsertTriangleNotRepeat(dtkID v0, dtkID v1,
                                                       dtkID v2) {
    // You must call SetPoints before excuting this function.
    dtkAssert(mPts.get() != NULL, ILLEGAL_STATE);
  ...
  dtkID dtkStaticTriangleMesh::RemoveTriangleDisordered(dtkID v0, dtkID v1,
                                                        dtkID v2) {
    for (unsigned int i = 0; i < mEC.size(); i++) {
      if (mEC[i].a == v0 && mEC[i].b == v1 && mEC[i].c == v2) {
        return RemoveTriangle(v0, v1, v2);
      }
      else if (mEC[i].a == v0 && mEC[i].b == v2 && mEC[i].c == v1) {
        return RemoveTriangle(v0, v2, v1);
      }
      else if (mEC[i].a == v1 && mEC[i].b == v0 && mEC[i].c == v2) {
        return RemoveTriangle(v1, v0, v2);
      }
      else if (mEC[i].a == v1 && mEC[i].b == v2 && mEC[i].c == v0) {
        return RemoveTriangle(v1, v2, v0);
      }
      else if (mEC[i].a == v2 && mEC[i].b == v0 && mEC[i].c == v1) {
        return RemoveTriangle(v2, v0, v1);
      }
      else if (mEC[i].a == v2 && mEC[i].b == v1 && mEC[i].c == v0) {
        return RemoveTriangle(v2, v1, v0);
      }
    }
    return 0;
  }

  dtkID dtkStaticTriangleMesh::modifyElement(dtkID srcIndex, int srcOrder,
                                             dtkID dest) {
  ...
  dtkID dtkStaticTriangleMesh::modifyElement(dtkID srcIndex, dtkID3 dest) {
    ...
    dtkAssert(dest.a < mPts->GetNumberOfPoints() &&
              dest.b < mPts->GetNumberOfPoints() &&
              dest.c < mPts->GetNumberOfPoints());
  ...
  bool dtkStaticTriangleMesh::IsModified() const { return mModified; }
  ...
  void dtkStaticTriangleMesh::Rebuild() {
    ...
    for (size_t i = 0; i < mEC.size(); ++i) {
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        dtkAssert(finishedEdge.find(key) == finishedEdge.end(),
                  MUST_BE_MANIFOLD_MESH);

        std::map<dtkDWORD, dtkID>::iterator pos;
        pos = hes.find(key);
        if (pos !=
            hes.end()) // a pair edge found, because edge with a, b are sorted.
        {
          // return the mapped value.
          dtkID he = pos->second;
          dtkID d, s;
          // decode the code of half-edge
          // d present triangle index, s present half-edge index in local
          // triangle.
          ...
          //-------------------------------------------------------------------------
          // there should be:
          // dthID he_ij = dtkStaticTriangleMesh::EncodeHE((dtkID)i, (dtkID)(j +
          // 1)); mE2E[d][s-1] = he_ij; mE2E[i][j]   = he;
          //-------------------------------------------------------------------------
          mE2E[d][s - 1] = (dtkID)i;
          mE2E[i][j] = d;
    ...
    for (std::map<dtkDWORD, dtkID>::iterator iter = hes.begin();
         iter != hes.end(); ++iter) {
    ...
    // if the triangle's direction is negetive, modify the mE2E and mEC.but why
    // not modify the mB2E?
    ...
    while (!detected.empty()) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        if (hes.find(Merge(sorted.a, sorted.b)) != hes.end())
          continue; // Is boundary edge, has been dealt before.
        ...
        dtkID idx;
        if (t == mEC[neighbour][0])
          idx = 1;
        if (t == mEC[neighbour][1])
          idx = 2;
        if (t == mEC[neighbour][2])
          idx = 3;
