...
/**
 * @file dtkPhysKnotPlanner.cpp
 * @brief dtkPhysKnotPlanner 实现
...
dtkPhysKnotPlanner::dtkPhysKnotPlanner(
    dtkPhysMassSpringThread::Ptr newSutureThread) {
...
void dtkPhysKnotPlanner::KnotRecognition(
  ...
  if (mDoKnotPlanning) {
    ...
    for (set<dtkID>::iterator itr = collisionSegments.begin();
         itr != collisionSegments.end(); itr++)
    ...
    for (dtkID i = 0; i < sortedCollisionSegments.size(); i++) {
      if (continuousSegmentStart == dtkIntMax) {
        ...
        continuousSegments.push_back(
            dtkID2(continuousSegmentStart, continuousSegmentEnd));
        continuousSegmentStart = continuousSegmentEnd =
            sortedCollisionSegments[i];
      }
    }
    ...
    for (dtkID i = 0; i + 1 < continuousSegments.size(); i++) {
      if (continuousSegments[i][1] - continuousSegments[i][0] > 1 &&
        ...
        mKnots.push_back(
            dtkID2(continuousSegments[i][0], continuousSegments[i + 1][1]));
  ...
  for (dtkID i = 0; i < mKnots.size(); i++) {
    ...
    for (dtkID j = 0; j < mSutureThread->GetNumberOfSegments(); j++) {
      ...
      dtkDouble3 trappedSegmentCenter =
          (mSutureThread->GetMassPoint(j * 2)->GetPosition() +
           mSutureThread->GetMassPoint(j * 2 + 2)->GetPosition()) *
          0.5;
    ...
    if (minSegID != dtkIntMax) {
      ...
      mAvoidIntervals.push_back(dtkInterval<int>((int)mSegmentInKnots[i] - 4,
                                                 (int)mSegmentInKnots[i] + 4));
    }
  }
...
void dtkPhysKnotPlanner::UpdateKnotOrientations() {
  for (dtkID i = 0; i < mKnots.size(); i++) {
    ...
    dtkPhysMassPoint *point1_2 =
        mSutureThread->GetMassPoint(segmentID_1 * 2 + 2);

    dtkPhysMassPoint *point2_1 = mSutureThread->GetMassPoint(segmentID_2 * 2);
    dtkPhysMassPoint *point2_2 =
        mSutureThread->GetMassPoint(segmentID_2 * 2 + 2);
...
void dtkPhysKnotPlanner::UpdateKnotCenterPoints() {
  for (dtkID i = 0; i < mKnots.size(); i++) {
    ...
    mKnotCenterPoints[i] =
        mKnotCenterPoints[i] +
        mSutureThread->GetMassPoint(mKnots[i][0] * 2)->GetPosition();
    mKnotCenterPoints[i] =
        mKnotCenterPoints[i] +
        mSutureThread->GetMassPoint(mKnots[i][1] * 2 + 2)->GetPosition();
    mKnotCenterPoints[i] =
        mKnotCenterPoints[i] +
        mSutureThread->GetMassPoint(mKnots[i][0] + mKnots[i][1] + 1)
            ->GetPosition();
...
void dtkPhysKnotPlanner::UpdateKnot(double timeslice) {
  ...
  for (dtkID i = 0; i < mKnots.size(); i++) {
    ...
    dtkPhysMassPoint *pointOnSegment_1 =
        mSutureThread->GetMassPoint(mSegmentInKnots[i] * 2);
    dtkPhysMassPoint *pointOnSegment_2 =
        mSutureThread->GetMassPoint(mSegmentInKnots[i] * 2 + 2);
    ...
    // tempPoint = (pointOnSegment_1->GetPosition() +
    // pointOnSegment_2->GetPosition()) * 0.5;
    ...
    // }
    dtkDouble3 vec =
        pointOnSegment_1_oldPos - segmentVec * percent - mKnotCenterPoints[i];
    ...
    pointOnSegment_1->SetVel(
        pointOnSegment_1->GetVel() +
        (pointOnSegment_1_newPos - pointOnSegment_1_oldPos) / timeslice);
    pointOnSegment_2->SetPosition(pointOnSegment_2_newPos);
    pointOnSegment_2->SetVel(
        pointOnSegment_2->GetVel() +
        (pointOnSegment_2_newPos - pointOnSegment_2_oldPos) / timeslice);
  }
}
...
void dtkPhysKnotPlanner::UpdateSegmentInKnot() {
  ...
  for (dtkID i = 0; i < mSegmentInKnots.size(); i++) {
    if (mSegmentInKnots[i] == dtkIntMax)
      continue;
    dtkDouble3 pointOnSegment_0 =
        mSutureThread->GetMassPoint(mSegmentInKnots[i] * 2)->GetPosition();
    dtkDouble3 pointOnSegment_1 =
        mSutureThread->GetMassPoint(mSegmentInKnots[i] * 2 + 2)->GetPosition();
    ...
    dtkDouble3 impulseVec(pointOnSegment_0 - tempSegmentOri * oriPercent -
                          knotCenterPoint);
    ...
    double percent = /*length( proj )*/ cosangle /
                     /*mSutureThread->GetInterval();*/ length(tempSegmentOri);
    /*if( percent > 0.1 )
            percent = 0.1;
    else if( percent < -0.1 )
            percent = -0.1;*/
