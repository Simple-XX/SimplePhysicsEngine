...
/**
 * @file dtkPhysMassSpring.cpp
 * @brief dtkPhysMassSpring 实现
...
namespace dtk {
  ...
  dtkPhysMassSpring::dtkPhysMassSpring(double defaultMass, double defaultStiff,
                                       double defaultDamp,
                                       double defaultPointDamp,
                                       double defaultPointResistence,
                                       dtkDouble3 defaultGravityAccel) {
  ...
  dtkPhysMassSpring::dtkPhysMassSpring(double defaultMass, double defaultStiff,
    ...
    mDefaultPointDamp = defaultPointDamp;
    mDefaultPointResistence = defaultPointResistence;
    mDefaultTimeStep = defaultTimeStep;
    mDefaultGravityAccel = defaultGravityAccel;
#ifdef DTK_CL
    mUseMultiThread = false;
    source_ = "";
  ...
  dtkPoints::Ptr dtkPhysMassSpring::GetPoints() { return mPts; }
  ...
  dtkID dtkPhysMassSpring::AddMassPoint(dtkID id, const double& mass,
                                        const dtkT3<double>& vel,
                                        double pointDamp, double pointResistence,
                                        dtkDouble3 defaultGravityAccel) {
    mMassPoints.push_back(new dtkPhysMassPoint(
      id, mPts, mass, vel, pointDamp, pointResistence, defaultGravityAccel));
  ...
  dtkID dtkPhysMassSpring::AddSpring(dtkID p1, dtkID p2, const double& stiff,
                                     const double& damp, const double& rest_length_factor) {
    if (p1 > p2)
      swap(p1, p2);
    if (mEdgeMap.find(dtkID2(p1, p2)) == mEdgeMap.end()) {
      dtkPhysSpring* newSpring =
        new dtkPhysSpring(mMassPoints[p1], mMassPoints[p2], stiff, damp);
      newSpring->SetRestLength(newSpring->GetOriLength() * rest_length_factor);
  ...
  bool dtkPhysMassSpring::Update_s(double timeslice, ItrMethod method,
                                   bool limitDeformation) {
    if (timeslice == 0)
      return true;
    switch (method) {
    case Euler:
  ...
  bool dtkPhysMassSpring::Update_iteration(double timeslice, ItrMethod method,
                                           dtkID iteration,
                                           bool limitDeformation) {
    if (timeslice == 0)
      return true;
  ...
  bool dtkPhysMassSpring::Update(double timeslice, ItrMethod method,
                                 bool limitDeformation) {
#ifdef DTK_CL
    if (mUseMultiThread)
  ...
  bool dtkPhysMassSpring::UpdateStrings(double timeslice, ItrMethod method,
                                        dtkID iteration, bool limitDeformation) {
    for (dtkID i = 0; i < mSprings.size(); i++)
  ...
  bool dtkPhysMassSpring::UpdateMassPoints(double timeslice, ItrMethod method,
                                           dtkID iteration) {
    for (dtkID i = 0; i < mMassPoints.size(); i++)
  ...
  bool dtkPhysMassSpring::PreUpdate(double timeslice, ItrMethod method,
                                    dtkID iteration) {
    return false;
  }
  ...
  void dtkPhysMassSpring::SetTimeStep(double timeStep) {
    mDefaultTimeStep = timeStep;
  }

  double dtkPhysMassSpring::GetTimeStep() {
    return mDefaultTimeStep;
  }
#ifdef DTK_CL
  bool dtkPhysMassSpring::Update_mt(double timeslice, ItrMethod method,
                                    bool limitDeformation) {
    if (timeslice == 0)
      return true;
    for (dtkID i = 0; i < 2; i++) {
  ...
  template <typename double> int dtkPhysMassSpring::SetupCL() {
    cl_int status;
    ...
    mSpringOriLengthBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfSprings() * sizeof(cl_float), mSpringOriLengthData, 0);
    mSpringStiffBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfSprings() * sizeof(cl_float), mSpringStiffData, 0);
    mSpringDampBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfSprings() * sizeof(cl_float), mSpringDampData, 0);

    mSpringVertexIDBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfSprings() * 2.0 * sizeof(cl_int),
                     mSpringVertexIDData, 0);
    mMPPosBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * 3.0 * sizeof(cl_float), mMPPosData, 0);

    mMPVelBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * 3 * sizeof(cl_float), mMPVelData, 0);

    mMPAccelBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * 3 * sizeof(cl_float), mMPAccelData, 0);
    mMPForceAccumBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
    ...
    mMPGravityBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * 3 * sizeof(cl_float), mMPGravityData, 0);
    mMPMassBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * sizeof(cl_float), mMPMassData, &status);
    ...
    mMPVelBufferBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfMassPoints() * 9 * sizeof(cl_float),
                     mMPVelBufferData, 0);
    mMPAccelBufferBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfMassPoints() * 9 * sizeof(cl_float),
                     mMPAccelBufferData, 0);
    mMPForceDecoratorBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfMassPoints() * 3 * sizeof(cl_float),
                     mMPForceDecoratorData, 0);
    mMPActiveBuffer = clCreateBuffer(
      context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
      this->GetNumberOfMassPoints() * sizeof(cl_bool), mMPActiveData, &status);
    mMPAdjacentVertexIDsBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfSprings() * sizeof(cl_int) * 2,
                     mMPAdjacentVertexIDsData, &status);
    mMPNumberOfAdjacentVertexBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfMassPoints() * sizeof(cl_int),
                     mMPNumberOfAdjacentVertexData, &status);
    this->CheckCLStatus(status, "numofadjvertexbuffer create buffer error!");
    mMPStartAdjacentVertexIDBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfSprings() * sizeof(cl_int),
                     mMPStartAdjacentVertexIDData, &status);
    mMPAdjacentSpringIDsBuffer =
      clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     this->GetNumberOfSprings() * sizeof(cl_int) * 2,
                     mMPAdjacentSpringIDsData, &status);
  ...
  int dtkPhysMassSpring::RunCLKernels(double timeslice, dtkID iteration) {
    ...
    status =
      clSetKernelArg(kernelMP, 0, sizeof(cl_int), (void*)&mNumberOfMassPoints);
    ...
    status =
      clSetKernelArg(kernelMP, 4, sizeof(cl_mem), (void*)&mMPForceAccumBuffer);
    // this->CheckCLStatus(status, "set kernel arg error! MP 4");
    status =
      clSetKernelArg(kernelMP, 5, sizeof(cl_mem), (void*)&mMPGravityBuffer);
    ...
    status =
      clSetKernelArg(kernelMP, 7, sizeof(cl_mem), (void*)&mMPPosBufferBuffer);
    // this->CheckCLStatus(status, "set kernel arg error! MP 7");
    status =
      clSetKernelArg(kernelMP, 8, sizeof(cl_mem), (void*)&mMPVelBufferBuffer);
    // this->CheckCLStatus(status, "set kernel arg error! MP 8");
    status = clSetKernelArg(kernelMP, 9, sizeof(cl_mem),
                            (void*)&mMPAccelBufferBuffer);
    // this->CheckCLStatus(status, "set kernel arg error! MP 9");
    status = clSetKernelArg(kernelMP, 10, sizeof(cl_mem),
                            (void*)&mMPForceDecoratorBuffer);
    // this->CheckCLStatus(status, "set kernel arg error! MP 10");
    status =
      clSetKernelArg(kernelMP, 11, sizeof(cl_mem), (void*)&mMPActiveBuffer);
    ...
    status = clSetKernelArg(kernelMP, 14, sizeof(cl_mem),
                            (void*)&mMPAdjacentVertexIDsBuffer);
    status = clSetKernelArg(kernelMP, 15, sizeof(cl_mem),
                            (void*)&mMPNumberOfAdjacentVertexBuffer);
    status = clSetKernelArg(kernelMP, 16, sizeof(cl_mem),
                            (void*)&mMPStartAdjacentVertexIDBuffer);
    status = clSetKernelArg(kernelMP, 17, sizeof(cl_mem),
                            (void*)&mMPAdjacentSpringIDsBuffer);
    status = clSetKernelArg(kernelMP, 18, sizeof(cl_mem),
                            (void*)&mSpringOriLengthBuffer);
    status =
      clSetKernelArg(kernelMP, 19, sizeof(cl_mem), (void*)&mSpringStiffBuffer);
    status =
      clSetKernelArg(kernelMP, 20, sizeof(cl_mem), (void*)&mSpringDampBuffer);

    work_size = this->GetNumberOfMassPoints();
    size_t local_work_size = 256;
    work_size = (work_size / local_work_size + 1) * local_work_size;
    clEnqueueNDRangeKernel(queue, kernelMP, 1, NULL, &work_size, &local_work_size,
                           0, NULL, NULL);
    // clFinish( queue );

    clEnqueueReadBuffer(queue, mMPPosBuffer, CL_TRUE, 0,
                        this->GetNumberOfMassPoints() * 3 * sizeof(cl_float),
                        mMPPosData, 0, NULL, NULL); //
    clEnqueueReadBuffer(queue, mMPVelBuffer, CL_TRUE, 0,
                        this->GetNumberOfMassPoints() * 3 * sizeof(cl_float),
                        mMPVelData, 0, NULL, NULL); //
    clEnqueueReadBuffer(queue, mMPPosBufferBuffer, CL_TRUE, 0,
                        this->GetNumberOfMassPoints() * 9 * sizeof(cl_float),
                        mMPPosBufferData, 0, NULL, NULL); //
    clEnqueueReadBuffer(queue, mMPVelBufferBuffer, CL_TRUE, 0,
                        this->GetNumberOfMassPoints() * 9 * sizeof(cl_float),
                        mMPVelBufferData, 0, NULL, NULL); //
    clFinish(queue);

    /*
    clEnqueueReadBuffer(queue, mMPAccelBuffer, CL_TRUE, 0,
            this->GetNumberOfMassPoints() * 3.0 * sizeof(cl_float), mMPAccelData,
    0, NULL, NULL ); clEnqueueReadBuffer(queue, mMPForceAccumBuffer, CL_TRUE, 0,
            this->GetNumberOfMassPoints() * 3.0 * sizeof(cl_float),
    mMPForceAccumData, 0, NULL, NULL ); // clEnqueueReadBuffer(queue,
    mMPGravityBuffer, CL_TRUE, 0, this->GetNumberOfMassPoints() * 3.0 *
    sizeof(cl_float), mMPGravityData, 0, NULL, NULL ); //
    clEnqueueReadBuffer(queue, mMPMassBuffer, CL_TRUE, 0,
            this->GetNumberOfMassPoints() * sizeof(cl_float), mMPMassData, 0,
    NULL, NULL ); // clEnqueueReadBuffer(queue, mMPAccelBufferBuffer, CL_TRUE, 0,
            this->GetNumberOfMassPoints() * 9.0 * sizeof(cl_float),
    mMPAccelBufferData, 0, NULL, NULL ); // clEnqueueReadBuffer(queue,
    mMPForceDecoratorBuffer, CL_TRUE, 0, this->GetNumberOfMassPoints() * 3.0 *
    sizeof(cl_float), mMPForceDecoratorData, 0, NULL, NULL ); //
    clEnqueueReadBuffer(queue, mMPActiveBuffer, CL_TRUE, 0,
            this->GetNumberOfMassPoints() * sizeof(cl_bool), mMPActiveData, 0,
    NULL, NULL ); // clEnqueueReadBuffer(queue, mSpringVertexIDBuffer, CL_TRUE, 0,
            this->GetNumberOfSprings() * 2.0 * sizeof(cl_int),
    mSpringVertexIDData, 0, NULL, NULL ); // clEnqueueReadBuffer(queue,
    mSpringOriLengthBuffer, CL_TRUE, 0, this->GetNumberOfSprings() *
    sizeof(cl_float), mSpringOriLengthData, 0, NULL, NULL ); //
    clEnqueueReadBuffer(queue, mSpringStiffBuffer, CL_TRUE, 0,
            this->GetNumberOfSprings() * sizeof(cl_float), mSpringStiffData, 0,
    NULL, NULL ); // clEnqueueReadBuffer(queue, mSpringDampBuffer, CL_TRUE, 0,
            this->GetNumberOfSprings() * sizeof(cl_float), mSpringDampData, 0,
    NULL, NULL ); //
            */
            /*
  ...
  void dtkPhysMassSpring::InitialCLArray() {
    ...
    for (dtkID i = 0; i < mSprings.size(); i++) {
      int id1, id2;
      id1 = mSpringVertexIDData[i * 2] =
        mSprings[i]->GetFirstVertex()->GetPointID();
      id2 = mSpringVertexIDData[i * 2 + 1] =
        mSprings[i]->GetSecondVertex()->GetPointID();

      tmpAdjacentVertexIDs[id1].push_back(
        mSprings[i]->GetSecondVertex()->GetPointID());
      tmpAdjacentVertexIDs[id2].push_back(
        mSprings[i]->GetFirstVertex()->GetPointID());
  ...
  */
    clEnqueueWriteBuffer(queue, mMPForceAccumBuffer, CL_TRUE, 0,
                         sizeof(cl_float) * this->GetNumberOfMassPoints() * 3,
                         mMPForceAccumData, 0, NULL, NULL);
    // clFinish(queue);
  }

  void dtkPhysMassSpring::UpdateCopyBack() {
    for (dtkID i = 0; i < mMassPoints.size(); i++) {
      dtkT3<double> vel(mMPVelData[i * 3], mMPVelData[i * 3 + 1],
                        mMPVelData[i * 3 + 2]);
      dtkT3<double> pos(mMPPosData[i * 3], mMPPosData[i * 3 + 1],
                        mMPPosData[i * 3 + 2]);
  ...
  void dtkPhysMassSpring::TransportForce(double timeslice) {
    ...
    for (dtkID i = 0; i < this->GetNumberOfMassPoints(); i++) {
      ...
      if (!point->IsActive()) {
        ...
        if (label != 0) {
          mTransportForces[label] =
            mTransportForces[label] + point->GetForceAccum();
        }
      }
    }
  }

  void dtkPhysMassSpring::SetTriangleMesh(
    dtkStaticTriangleMesh::Ptr newTriangleMesh) {
    this->SetPoints(newTriangleMesh->GetPoints());
    for (dtkID i = 0; i < newTriangleMesh->GetNumberOfPoints(); i++) {
      this->AddMassPoint(i, mDefaultMass, dtkT3<double>(0, 0, 0),
                         mDefaultPointDamp, mDefaultPointResistence,
                         mDefaultGravityAccel);
    }
