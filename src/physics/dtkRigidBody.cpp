...
/**
 * @file dtkRigidBody.cpp
 * @brief dtkRigidBody 实现
...
dtkRigidBody::dtkRigidBody(unsigned int id, double mass)
    : mId(id), mRotation(glm::mat2x2(1.0)), mAngularVelocity(0.0), mTorque(0.0),
      mFriction(1.0) {
  set_mass(mass);
}
...
// 更新动量
void dtkRigidBody::update_impulse(const dtkDouble2 &impulse,
                                  const dtkDouble2 &r) {
  if (std::isinf(mMass))
    return;
...
unsigned int dtkRigidBody::get_id() const { return mId; }

double dtkRigidBody::get_mass() const { return mMass; }
...
double dtkRigidBody::get_inv_mass() const { return invMass; }

double dtkRigidBody::get_inertia() const { return mInertia; }
...
double dtkRigidBody::get_inv_inertia() const { return invInertia; }

const dtkDouble2 &dtkRigidBody::get_centroid() const { return mCentroid; }

void dtkRigidBody::set_centroid(const dtkDouble2 &centroid) {
  mCentroid = centroid;
}

const dtkDouble2 &dtkRigidBody::get_position() const { return mPosition; }

void dtkRigidBody::set_position(const dtkDouble2 &position) {
  mPosition = position;
}

const dtkMatrix22 &dtkRigidBody::get_rotation() const { return mRotation; }

void dtkRigidBody::set_rotation(const dtkMatrix22 &rotation) {
  mRotation = rotation;
}

const dtkDouble2 &dtkRigidBody::get_velocity() const { return mVelocity; }

void dtkRigidBody::set_velocity(const dtkDouble2 &velocity) {
  mVelocity = velocity;
}

double dtkRigidBody::get_angular_velocity() const { return mAngularVelocity; }

void dtkRigidBody::set_angular_velocity(double angular_velocity) {
  mAngularVelocity = angular_velocity;
}

const dtkDouble2 &dtkRigidBody::get_force() const { return mForce; }

void dtkRigidBody::set_force(const dtkDouble2 &force) { mForce = force; }

double dtkRigidBody::get_torque() const { return mTorque; }

void dtkRigidBody::set_torque(double torque) { mTorque = torque; }

double dtkRigidBody::get_friction() const { return mFriction; }

void dtkRigidBody::set_friction(double friction) { mFriction = friction; }
...
// 计算多边形重心
static dtkDouble2
calc_polygon_centroid(const std::vector<dtkDouble2> &vertices) {
  dtkDouble2 gc;
  auto size = vertices.size();
  // 重心 = (各三角形重心 * 其面积) / 总面积
  // 三角形重心 = 两向量之和 / 3
...
// 计算多边形转动惯量
static double calc_polygon_inertia(double mass,
                                   const dtkRigidBody::vertex_list &vertices) {
...
dtkPolygonRigidBody::dtkPolygonRigidBody(
    unsigned int id, double mass, const dtkRigidBody::vertex_list &vertices)
...
// 轴：只需采用两个凸包的每个条做检测即可
double dtkPolygonRigidBody::min_separating_axis(
    size_t &idx, const dtkPolygonRigidBody &other) const {
...
double dtkPolygonRigidBody::SAT(size_t &idx,
                                const dtkPolygonRigidBody &other) const {
  double concave_separation = inf;
  std::vector<index_list> polyList1 = mPolygonList,
                          polyList2 = other.mPolygonList;
  int poly_cnt = mPolygonList.size(),
      poly_cnt_other = other.mPolygonList.size();
  for (int i = 0; i < poly_cnt; i++) {
    ...
    for (int j = 0; j < poly_cnt_other; j++) {
      ...
      for (int m = 0; m < p_cnt1; m++) {
        ...
        // 获得当前顶点到下一顶点的边的单位法向量
        dtkDouble2 edge = (*this)[poly_index1[(m + 1) % poly_index1.size()]] -
                          (*this)[poly_index1[m]];
        auto N = normal(edge);
        // 最小分离向量
        auto min_sep = inf;
...
bool dtkPolygonRigidBody::isConcavePolygon(index_list iVertices) {
  ...
  for (int i = 0; i < len; i++) {
    dtkDouble2 v1 =
        mVertices[iVertices[i]] - mVertices[iVertices[(i - 1 + len) % len]];
    dtkDouble2 v2 =
        mVertices[iVertices[(i + 1) % len]] - mVertices[iVertices[i]];
...
dtkPolygonRigidBody::divideConcavePolygon(index_list iVertices) {
  ...
  for (i = 0; i < len; i++) {
    ...
    dtkDouble2 vAxis = mVertices[iVertices[(i + 1) % len]] -
                       mVertices[iVertices[i]],
               v = mVertices[iVertices[(i + 2) % len]] -
                   mVertices[iVertices[i]];
  ...
  if (isConcavePolygon(polygon1)) {
    ...
    polygon_list.insert(polygon_list.end(), sub_polygon_list.begin(),
                        sub_polygon_list.end());
  } else {
    polygon_list.push_back(polygon1);
  }
...
std::vector<index_list>
dtkPolygonRigidBody::decomposeConcavePolygon(index_list iVertices) {
  ...
  for (int i = 0; i < return_list.size(); i++) {
    ...
    for (int j = 2; j < len1; j++) {
      if ((mVertices[return_list[i][j]].x - mVertices[return_list[i][0]].x) *
              (mVertices[return_list[i][j]].y -
               mVertices[return_list[i][1]].y) !=
          (mVertices[return_list[i][j]].x - mVertices[return_list[i][1]].x) *
              (mVertices[return_list[i][j]].y -
               mVertices[return_list[i][0]].y)) {
        colineation = false;
        break;
      }
    }
