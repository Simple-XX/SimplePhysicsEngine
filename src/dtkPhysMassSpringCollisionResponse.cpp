...
/**
 * @file dtkPhysMassSpringCollisionResponse.cpp
 * @brief dtkPhysMassSpringCollisionResponse 实现
...
void dtkPhysMassSpringCollisionResponse::Update(
    double timeslice,
  ...
  for (dtkID i = 0; i < intersectResults.size(); i++) {
    ...
    if (type1 == dtkCollisionDetectPrimitive::SEGMENT) {
      if (type2 == dtkCollisionDetectPrimitive::SEGMENT) { // 线段与线段
        responseType = SEGMENT_SEGMENT;
      } else if (type2 ==
                 dtkCollisionDetectPrimitive::TRIANGLE) { // 线段与三角形
        assert(false); // only tri - seg pairs are possible
      } else
        assert(false);
    } else if (type1 == dtkCollisionDetectPrimitive::TRIANGLE) {
      if (type2 == dtkCollisionDetectPrimitive::SEGMENT) { // 三角形与段
        responseType = TRIANGLE_SEGMENT;
      } else if (type2 ==
                 dtkCollisionDetectPrimitive::TRIANGLE) { // 三角形与三角形
    ...
    case SEGMENT_SEGMENT: {
      ...
      dtkPhysMassPoint *massPoint11 =
          mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[0]);
      dtkPhysMassPoint *massPoint12 =
          mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[1]);
      dtkPhysMassPoint *massPoint21 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[0]);
      dtkPhysMassPoint *massPoint22 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[1]);
      ...
      double penetrate1 =
          dot(oriPos21 - oriPos11,
              normalize(cross(oriPos12 - oriPos11, oriPos22 - oriPos21)));
      ...
      double penetrate2 =
          dot(Pos21 - Pos11, normalize(cross(Pos12 - Pos11, Pos22 - Pos21)));

      // cout << penetrate1 << " " << penetrate2 << endl;
      //  relative relationship of two segments invert.
      //  if relationship of two segments changes, normal direction and length
      //  also changed.
      if (penetrate1 * penetrate2 < 0) {
        // cout << normal << endl;
        // cout << "invert- " << endl;
        normal = GK::Normalize(-normal) *
                 (pri_1->GetExtend() * 2.0 + pri_2->GetExtend() * 2.0 -
                  GK::Length(normal) / 2.0) *
                 2.0;
        // cout << normal << endl;
      }
      ...
      dtkDouble3 velRelative =
          vel21 * uv2[0] + vel22 * uv2[1] - vel11 * uv1[0] - vel12 * uv1[1];

      dtkDouble3 impulse_normal = normalize(impulse);
      double relativeVel = dot(velRelative, impulse_normal);
      double candidate1 =
          -relativeVel * (massPoint11->GetMass() + massPoint12->GetMass() +
                          massPoint21->GetMass() + massPoint22->GetMass());
      ...
      // impulse = impulse_normal * impulse_value / ( 1 + pow( uv1[0], 2 ) +
      // pow( uv1[1], 2 ) + pow( uv2[0], 2 ) + pow( uv2[1], 2 ) );

      // the process is that assign impluse to each mass point.
      impulse = impulse / (1 + pow(uv1[0], 2) + pow(uv1[1], 2) +
                           pow(uv2[0], 2) + pow(uv2[1], 2));
      ...
      // mMassSprings[pri_1->mMajorID]->ImpulsePropagate( impulse * (-uv1[0]
      // / 1.0), pri_1->mDetailIDs[0], 0 );
      // mMassSprings[pri_1->mMajorID]->ImpulsePropagate( impulse * (-uv1[1]
      // / 1.0), pri_1->mDetailIDs[1], 0 );
      // mMassSprings[pri_2->mMajorID]->ImpulsePropagate( impulse * (uv2[0]
      // / 1.0), pri_2->mDetailIDs[0], 0 );
      // mMassSprings[pri_2->mMajorID]->ImpulsePropagate( impulse * (uv2[1]
      // / 1.0), pri_2->mDetailIDs[1], 0 );

      // four mass points to two mass points???
      dtkPhysMassPoint *massPoint1112 =
          mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[0] + 1);
      dtkPhysMassPoint *massPoint2122 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[0] + 1);
      ...
      if (pri_1->mDetailIDs[0] > 0) {
        dtkPhysMassPoint *massPoint1011 =
            mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[0] -
                                                        1);
        massPoint1011->AddImpulse(impulse *
                                  (-uv1[0] * 0.5 / massPoint1011->GetMass()));
      }
      if (pri_1->mDetailIDs[1] + 1 <
          mMassSprings[pri_1->mMajorID]->GetNumberOfMassPoints()) {
        dtkPhysMassPoint *massPoint1213 =
            mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[1] +
                                                        1);
        massPoint1213->AddImpulse(impulse *
                                  (-uv1[1] * 0.5 / massPoint1213->GetMass()));
      }

      if (pri_2->mDetailIDs[0] > 0) {
        dtkPhysMassPoint *massPoint2021 =
            mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[0] -
                                                        1);
        massPoint2021->AddImpulse(impulse *
                                  (uv2[0] * 0.5 / massPoint2021->GetMass()));
      }
      if (pri_2->mDetailIDs[1] + 1 <
          mMassSprings[pri_2->mMajorID]->GetNumberOfMassPoints()) {
        dtkPhysMassPoint *massPoint2223 =
            mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[1] +
                                                        1);
        massPoint2223->AddImpulse(impulse *
                                  (uv2[1] * 0.5 / massPoint2223->GetMass()));
      }
      break;
    }
    case TRIANGLE_SEGMENT: {
      ...
      for (dtkID pierceID = 0; pierceID < mPierceSegments.size(); pierceID++) {
        if (pri_2->mMajorID == mPierceSegments[pierceID][0] &&
            ...
            (GK::Length(normal)) * uv2[0] >
                (pri_1->GetExtend() + pri_2->GetExtend())) {
      ...
      dtkPhysMassPoint *massPoint12 =
          mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[1]);
      dtkPhysMassPoint *massPoint13 =
          mMassSprings[pri_1->mMajorID]->GetMassPoint(pri_1->mDetailIDs[2]);
      dtkPhysMassPoint *massPoint21 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[0]);
      dtkPhysMassPoint *massPoint22 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[1]);
      ...
      impulse = impulse * 2.0 /
                (pow(uvw1[0], 2) + pow(uvw1[1], 2) + pow(uvw1[2], 2) +
                 pow(uv2[0], 2) + pow(uv2[1], 2));
      ...
      // dtkDouble3 vel11 = massPoint11->GetVel();
      //{
      //	dtkDouble3 temp = vel11 - impulse_normal * dot( vel11,
      //impulse_normal ); 	dtkDouble3 impulse11 = impulse * ( -uvw1[0] /
      //massPoint11->GetMass() ); 	if( length(temp) > 0.01 )
      //	{
      //		if( length(impulse11) * coe > length( temp ) )
      //		{
      //			impulse11 = impulse11 - temp *
      //massPoint11->GetMass();
      //		}
      //		else
      //		{
      //			impulse11 = impulse11 - normalize( temp ) *
      //length(impulse11) * coe * massPoint11->GetMass();
      //		}
      //	}
      ...
      // dtkDouble3 vel12 = massPoint12->GetVel();
      //{
      //	dtkDouble3 temp = vel12 - impulse_normal * dot( vel12,
      //impulse_normal ); 	dtkDouble3 impulse12 = impulse * ( -uvw1[1] /
      //massPoint12->GetMass() ); 	if( length(temp) > 0.01 )
      //	{
      //		if( length(impulse12) * coe > length( temp ) )
      //		{
      //			impulse12 = impulse12 - temp *
      //massPoint12->GetMass();
      //		}
      //		else
      //		{
      //			impulse12 = impulse12 - normalize( temp ) *
      //length(impulse12) * coe * massPoint12->GetMass();
      //		}
      //	}
      ...
      //{
      //	dtkDouble3 temp = vel13 - impulse_normal * dot( vel13,
      //impulse_normal ); 	dtkDouble3 impulse13 = impulse * ( -uvw1[2] /
      //massPoint13->GetMass() ); 	if( length(temp) > 0.01 )
      //	{
      //		if( length(impulse13) * coe > length( temp ) )
      //		{
      //			impulse13 = impulse13 - temp *
      //massPoint13->GetMass();
      //		}
      //		else
      //		{
      //			impulse13 = impulse13 - normalize( temp ) *
      //length(impulse13) * coe * massPoint13->GetMass();
      //		}
      //	}
      ...
      // dtkDouble3 vel21 = massPoint21->GetVel();
      //{
      //	dtkDouble3 temp = vel21 - impulse_normal * dot( vel21,
      //impulse_normal ); 	dtkDouble3 impulse21 = impulse * ( uv2[0] /
      //massPoint21->GetMass() ); 	if( length(temp) > 0.01 )
      //	{
      //		if( length(impulse21) * coe > length( temp ) )
      //		{
      //			impulse21 = impulse21 - temp *
      //massPoint21->GetMass();
      //		}
      //		else
      //		{
      //			impulse21 = impulse21 - normalize( temp ) *
      //length(impulse21) * coe * massPoint21->GetMass();
      //		}
      //	}
      ...
      // dtkDouble3 vel22 = massPoint22->GetVel();
      //{
      //	dtkDouble3 temp = vel22 - impulse_normal * dot( vel22,
      //impulse_normal ); 	dtkDouble3 impulse22 = impulse * ( uv2[1] /
      //massPoint22->GetMass() ); 	if( length(temp) > 0.01 )
      //	{
      //		if( length(impulse22) * coe > length( temp ) )
      //		{
      //			impulse22 = impulse22 - temp *
      //massPoint22->GetMass();
      //		}
      //		else
      //		{
      //			impulse22 = impulse22 - normalize( temp ) *
      //length(impulse22) * coe * massPoint22->GetMass();
      //		}
      //	}
    ...
    case TRIANGLE_TRIANGLE: {
      ...
      dtkPhysMassPoint *massPoint22 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[1]);
      dtkPhysMassPoint *massPoint23 =
          mMassSprings[pri_2->mMajorID]->GetMassPoint(pri_2->mDetailIDs[2]);
      ...
      impulse = impulse * 2.0 /
                (pow(uvw1[0], 2) + pow(uvw1[1], 2) + pow(uvw1[2], 2) +
                 pow(uvw2[0], 2) + pow(uvw2[1], 2) + pow(uvw2[2], 2));
