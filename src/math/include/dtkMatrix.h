...
namespace dtk {
   ...
   * 实现任意维的矩阵
   */
  template <class T, class MatrixT = std::vector<T>> struct dtkMatrix {
    ...
    dtkMatrix(void) : ni(0), nj(0) {}

    dtkMatrix(int ni_, int nj_) : ni(ni_), nj(nj_), a(ni_* nj_) {
      assert(ni_ >= 0 && nj >= 0);
    }

    dtkMatrix(int ni_, int nj_, MatrixT& a_) : ni(ni_), nj(nj_), a(a_) {
      assert(ni_ >= 0 && nj >= 0);
    }
  ...
  typedef dtkMatrix<unsigned long long, std::vector<unsigned long long>>
    dtkMatrixULLong;
