...
public:
   ...
   * @note	 从文件获取点集新建弹簧图元.
   */
  void CreateMassSpring(const char *filename, dtkID id, double point_mass,
                        double stiffness, double damp, double pointDamp,
                        double pointResistence, dtkDouble3 gravityAccel,
   ...
   * @note	 从文件获取点集新建弹簧图元.
   */
  void CreateTriangleMassSpring(const char *filename, dtkID id,
   ...
   * @note	 从文件获取点集新建弹簧图元.
   */
  void CreateTetraMassSpring(
      const char *filename, dtkID id, double point_mass, double stiffness,
      double damp, double pointDamp, double pointResistence,
      dtkDouble3 gravityAccel, /*bool surface = true,*/
      dtkStaticMeshEliminator::MeshEliminatorResultsCallback callback = 0,
      void *pContext =
          0); // tetraMassSpringType: 1 for surface, 2 for body, 3 for all
  void DestroyTetraMassSpring(dtkID id);

  void CreateSutureThread(dtkID id, double length, dtkDouble3 firstPointPos,
                          dtkPhysMassSpringThread::Orientation orientation,
                          double mass, double edgeStiff, double bendStiff,
                          double torsionStiff, double edgeDamp,
                          double extraEdgeDamp, double bendDamp,
                          double torsionDamp, double interval = 2.0,
                          double radius = 0.8,
                          double selfCollisionStrength = 20000.0);
  ...
  void DestroyCollisionResponse(dtkID object1_id,
  ...
  GetCollisionDetectHierarchy(dtkID id, CollisionHierarchyType type);
  ...
  void ExecuteCustomDetectTriangleArea(
  ...
  dtkPhysParticleSystem::Ptr CreateParticleSystem(dtkID id,
                                                  double particleRadius,
                                                  double particleMass,
                                                  double particleLifetime);
  void DestroyParticleSystem(dtkID id);

  void CreateObstacleForParticleSystem(
  ...
  void DestroyObstacleForParticleSystem(dtkID particlesystem_id,
                                        dtkID object_id);
...
private:
  ...
  dtkID AllocateDetails(
      const std::multimap<dtkID, dtkID, std::greater<dtkID>> &sortedMap,
      size_t average);
...
public:
  ...
  std::map<dtkID, dtkCollisionDetectHierarchyKDOPS::Ptr>
      mCollisionDetectHierarchies;
  std::map<dtkID, dtkCollisionDetectHierarchyKDOPS::Ptr>
      mThreadCollisionDetectHierarchies;
  std::map<dtkID, dtkCollisionDetectHierarchyKDOPS::Ptr>
      mInteriorCollisionDetectHierarchies;
  std::map<dtkID, dtkCollisionDetectHierarchyKDOPS::Ptr>
      mThreadHeadCollisionDetectHierarchies;
  ...
  // Response
  std::map<dtkID, CollisionResponseSet>
      mCollisionDetectResponseSets; /**< 碰撞响应集. */
  std::map<dtkID, CollisionResponseSet>
      mInternalCollisionDetectResponseSets;   /**< 内部碰撞响应集. */
...
public:
  ...
  // Allocator
  std::vector<std::vector<std::vector<dtkID>>>
      mAllocator;               /**< different id group to different threads */
