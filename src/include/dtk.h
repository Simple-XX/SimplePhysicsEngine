 ...
 * - points container, used in many graphical element in DTK, such as Mesh,
 * Graph dtkPointsReader dtkPointsWriter
 ...
 * The Dynamic Mesh is designed for maintaining the quality of the surface
 * meshes in the dynamic scenes and support the typical query requirements of
 * the virtual surgery systems.
 *
 *  The proposed solution bases on loose r-sample theory. It can effectively
 * protect the surface meshes' quality during the objects deformation, by
 * maintaining the loose r-sample properties of the surface sampling points set
 * S, the dynamic Delaunay triangulation of S, and the labels of the Delaunay
 * cells. In addition, loose r-sample based dynamic mesh allowing users to use
 * point operations instead of mesh operations. It also delivers an uniform data
 * structure which supports the typical mesh operations and greatly reduces the
 * difficulty of interactive algorithm design for virtual surgery systems.
 *
 *  The proposed mesh reconstruction algorithm and loose r-sample based scene
 *  management schema are the inheritance and development of the theory of
 * restricted Delaunay triangulation, r-sample based surface reconstruction
 * algorithm and loose r-sample based surface reconstruction algorithm. The
 * proposed mesh reconstruction algorithm extends the r-sample based surface
 * reconstruction algorithm to solid objects, and generates the tetrahedral
 * meshes; the proposed loose r-sample based scene management schema applies the
 * loose r-sample related theory to keep the quality fo the dynamic meshes.
 *
 * \section part4_sec Part 4 Mass-Spring & Rigid-body Simulation
 *  This part starts from a dtkScene which is designed for controlling multiple
 * physical simulations. It allows different types of dtkSceneObject to act.
 ...
 *      - for storing varies datas during the simulation and allowing other
 * parts do further computation, such as rendering.
 *      .
 *  .
 *
 *  - dtkSceneDriver
 *      - as a external participant in the simulation. Thus, you can interact
 * with this system by GUI and so on.
 *      .
 *  .
 */
