...
/**
 * @file dtkPhysTetraMassSpring.cpp
 * @brief dtkPhysTetraMassSpring 实现
...
const dtkID dtkPhysTetraMassSpring::tetra_order[7][4] = {
    ...
    {2, 3, 0, 1}, {1, 0, 3, 2}, {0, 1, 2, 3}, {0, 1, 3, 2}, // edge to edge
    {0, 2, 1, 3}, {0, 3, 2, 1}};

dtkPhysTetraMassSpring::dtkPhysTetraMassSpring(
    bool fullUseAltSpring, double defaultMass, double defaultStiff,
    double defaultDamp, double defaultPointDamp, double defaultPointResistence,
    dtkDouble3 defaultGravityAccel)
    : dtkPhysMassSpring(defaultMass, defaultStiff, defaultDamp,
...
bool dtkPhysTetraMassSpring::PreUpdate(double timeslice, ItrMethod method,
                                       dtkID iteration) {
  for (int i = 0; i < mTetraMesh->GetTetraNum(); i++) {
    ...
    if (tempTetra[0] != dtkErrorID) {
      ...
      if( GK::IsDegenerate( GK::Tetrahedron3(
                      GK::Point3( positions[0][0], positions[0][1],
      positions[0][2] ), GK::Point3( positions[1][0], positions[1][1],
      positions[1][2] ), GK::Point3( positions[2][0], positions[2][1],
      positions[2][2] ), GK::Point3( positions[3][0], positions[3][1],
      positions[3][2] ) ) ) )
      {
          assert(false);
          continue;
      ...
      */
      ...
      for (dtkID j = 0; j < 4; j++) {
        ...
        dtkT3<double> v_side_edge =
            positions[tetra_order[j][3]] - positions[tetra_order[j][1]];
        ...
        dtkT3<double> uvw = barycentricWeight(
            positions[tetra_order[j][3]], positions[tetra_order[j][0]],
            positions[tetra_order[j][1]], positions[tetra_order[j][2]]);
        if (uvw[0] > 0 && uvw[0] < 1 && uvw[1] > 0 && uvw[1] < 1 &&
            uvw[2] > 0 && uvw[2] < 1) {
          dtkT3<double> stiffForce =
              n_area_unit *
              ((oriLength - curLength) * mAltitudeStiff /
               oriLength); // 弹力
          ...
          massPoints[tetra_order[j][2]]->AddForce(force *
                                                  ((-1.0) * uvw[2])); // 力
#ifdef DTK_PHYSTETRAMASSSPRINGIMPL_DEBUG
          // cout << "get altitude spring." << endl;
          //  compute the two mass position of spring
          mAltitudeSpringPositions[i].first =
              positions[tetra_order[j][3]] - n_area_unit * curLength;
      ...
      for (dtkID j = 4; j < 7; j++) {
        ...
        if (uv_0[0] >= 0 && uv_0[0] <= 1 && uv_0[1] >= 0 && uv_0[1] <= 1 &&
          ...
          dtkT3<double> stiffForce = n_area_unit * ((oriLength - curLength) *
                                                    mAltitudeStiff / oriLength);
          dtkT3<double> vP0 = vels[tetra_order[j][0]] * uv_0[0] +
                              vels[tetra_order[j][1]] * uv_0[1];
          dtkT3<double> vP1 = vels[tetra_order[j][2]] * uv_1[0] +
                              vels[tetra_order[j][3]] * uv_1[1];
...
void dtkPhysTetraMassSpring::SetTetraMesh(
    dtkStaticTetraMesh::Ptr newTetraMesh) {
  ...
  for (dtkID i = 0; i < pts->GetNumberOfPoints(); i++) {
    ...
    this->AddMassPoint(i, this->mDefaultMass, dtkDouble3(0, 0, 0),
  ...
  for (dtkID i = 0; i < tempECTable.size(); i++) {
    ...
    for (int x = 0; x < 4; x++) {
      for (int y = x + 1; y < 4; y++) {
        this->AddSpring(tempTetra[x], tempTetra[y], this->mDefaultStiff,
                        this->mDefaultDamp);
      ...
      ori_positions[x] =
          dtkT3<double>(gk_oripoint.x(), gk_oripoint.y(), gk_oripoint.z());
    }
    ...
    for (dtkID j = 0; j < 7; j++) {
      if (j < 4) { // 相对底面的高
        dtkT3<double> ori_n_area = cross(ori_positions[tetra_order[j][1]] -
                                             ori_positions[tetra_order[j][0]],
                                         ori_positions[tetra_order[j][2]] -
                                             ori_positions[tetra_order[j][1]]);
        dtkT3<double> ori_v_side_edge =
            ori_positions[tetra_order[j][3]] - ori_positions[tetra_order[j][1]];
        ...
        dtkT3<double> ori_n_area = cross(ori_positions[tetra_order[j][1]] -
                                             ori_positions[tetra_order[j][0]],
                                         ori_positions[tetra_order[j][3]] -
                                             ori_positions[tetra_order[j][2]]);
        dtkT3<double> ori_v_side_edge =
            ori_positions[tetra_order[j][2]] - ori_positions[tetra_order[j][0]];
        dtkT3<double> ori_n_area_unit = normalize(ori_n_area);
...
#ifdef DTK_PHYSTETRAMASSSPRINGIMPL_DEBUG
    mAltitudeSpringPositions.push_back(pair<dtkT3<double>, dtkT3<double>>(
        dtkT3<double>(0, 0, 0), dtkT3<double>(0, 0, 0)));
#endif
  }
}
...
void dtkPhysTetraMassSpring::AddTetra(dtkID i) {
  ...
  for (int x = 0; x < 4; x++) {
    for (int y = x + 1; y < 4; y++) {
      this->AddSpring(tetra[x], tetra[y], this->mDefaultStiff,
                      this->mDefaultDamp);
      dtkID2 edge(tetra[x], tetra[y]);
      if (edge[0] > edge[1])
        swap(edge[0], edge[1]);
...
// template<typename T>
// dtkT3<T> dtkPhysTetraMassSpring<T>::GetBarycentricWeight(dtkT3<T> point1,
// dtkT3<T> point2, dtkT3<T> point3, dtkT3<T> point4)
//   {
...
//       if(detA == 0)
//       {
//           T D1 = (point2[2] - point1[2]) * (point3[0] - point1[0]) -
//           (point3[2] - point1[2]) * (point2[0] - point1[0]); if(D1 != 0)
//           {
//               uvw[1] = ((point4[2] - point1[2]) * (point3[0] - point1[0]) -
//               (point3[2] - point1[2]) * (point4[0] - point1[0])) / D1; uvw[2]
//               = ((point2[2] - point1[2]) * (point4[0] - point1[0]) -
//               (point4[2] - point1[2]) * (point2[0] - point1[0])) / D1; uvw[0]
//               = 1 - uvw[1] - uvw[2];
//           }
//           else
//           {
//               T D2 = ((point2[1] - point1[1]) * (point3[0] - point1[0]) -
//               (point3[1] - point1[1]) * (point2[0] - point1[0])); uvw[1] =
//               ((point4[0] - point1[0]) * (point3[1] - point1[1]) - (point3[0]
//               - point1[0]) * (point4[1] - point1[1])) / D2; uvw[2] =
//               ((point2[0] - point1[0]) * (point4[1] - point1[1]) - (point4[0]
//               - point1[0]) * (point2[1] - point1[1])) / D2; uvw[0] = 1 -
//               uvw[1] - uvw[2];
//           }
//       }
//       else
