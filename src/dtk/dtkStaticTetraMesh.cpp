...
/**
 * @file dtkStaticTetraMesh.cpp
 * @brief dtkStaticTetraMesh 实现
...
const dtkID dtkStaticTetraMesh::EAdj[4][3] = {
    {17, 13, 9}, {4, 12, 18}, {6, 16, 10}, {5, 8, 14}};

dtkStaticTetraMesh::dtkStaticTetraMesh() : mModified(false) {
  // nothing.
}
...
dtkPoints::Ptr dtkStaticTetraMesh::GetPoints() { return mPts; }
...
dtkID dtkStaticTetraMesh::InsertTetra(dtkID v0, dtkID v1, dtkID v2, dtkID v3,
...
dtkID dtkStaticTetraMesh::InsertTetra(const dtkID4 &tetra,
                                      bool allowDegenerate) {
...
dtkID dtkStaticTetraMesh::modifyElement(dtkID srcIndex, int srcOrder,
                                        dtkID dest) {
...
void dtkStaticTetraMesh::Modified() { mModified = true; }

bool dtkStaticTetraMesh::IsModified() { return mModified; }
...
void dtkStaticTetraMesh::Rebuild() {
  ...
  mV2F.resize(mPts->GetMaxID() + 1, dtkErrorID);
  mF2F.clear();
  mF2F.resize(mEC.size(),
              dtkID4(dtkErrorID, dtkErrorID, dtkErrorID, dtkErrorID));
  mB2F.clear();
  ...
  for (dtkID cell = 0; cell < cellNum; ++cell) {
    ...
    dtkID3 tris[5] = {
        dtkID3(dtkErrorID, dtkErrorID, dtkErrorID),
        dtkID3(verts.a, verts.c, verts.b), dtkID3(verts.a, verts.b, verts.d),
        dtkID3(verts.b, verts.c, verts.d), dtkID3(verts.c, verts.a, verts.d)};
...
bool dtkStaticTetraMesh::IsValidTetra(dtkID4 verts, dtkPoints::Ptr pts) {
...
bool dtkStaticTetraMesh::GetSurface(dtkStaticTriangleMesh::Ptr &triMesh) {
  ...
  for (dtkID i = 0; i < mEC.size(); i++) {
    ...
    dtkID3 tempOriFace;
    for (dtkID j = 0; j < 4; j++) {
      tempOriFace = dtkID3(tetra[this->EA2V[j][0]], tetra[this->EA2V[j][1]],
                           tetra[this->EA2V[j][2]]);
      tempSortedFace =
          this->SortTriVertex(tempOriFace[0], tempOriFace[1], tempOriFace[2]);
