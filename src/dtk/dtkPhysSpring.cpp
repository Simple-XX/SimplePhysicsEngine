...
/**
 * @file dtkPhysSpring.cpp
 * @brief dtkPhysSpring 实现
...
dtkPhysSpring::dtkPhysSpring(dtkPhysMassPoint *p1, dtkPhysMassPoint *p2,
                             const double &stiff, const double &damp) {
  mVerteces[0] = p1;
  mVerteces[1] = p2;
...
bool dtkPhysSpring::Update(double timeslice, ItrMethod method, dtkID iteration,
                           bool limitDeformation) {
  // 方向向量
  dtkT3<double> vec = mVerteces[1]->GetPosition(method, iteration) -
                      mVerteces[0]->GetPosition(method, iteration);
...
#ifdef DTK_PHYSSPRINGIMPL_DEBUG
      cout<<"v0 after modification: "<<(v0-v * 0.5)[0]<<" "<<(v0-v * 0.5)[1]<<"
"<<(v0-v * 0.5)[2]<<endl; cout<<"v1 after modification: "<<(v1-v * 0.5)[0]<<"
"<<(v1-v * 0.5)[1]<<" "<<(v1-v * 0.5)[2]<<endl; #endif //
DTK_PHYSSPRINGIMPL_DEBUG
*/
      ...
      T t3 = dot(vec1,vec2);
      T t1 = ((-2.0) * t3 + sqrt(4.0 * t3 * t3 - 4.0 * dot(vec2,vec2) *
      (dot(vec1,vec1) - 1.05 * 1.05 * mOriLength * mOriLength))) / (2.0 *
      dot(vec2,vec2)); T t2 = ((-2.0) * t3 - sqrt(4.0 * t3 * t3 - 4.0 *
      dot(vec2,vec2) * (dot(vec1,vec1) - 1.05 * 1.05 * mOriLength *
      mOriLength))) / (2.0 * dot(vec2,vec2)); if(t2 > 0) t = t2; else t = t1;
      ...
      cout<<"v0 after modification: "<<(v0-u1 * t)[0]<<" "<<(v0-u1 * t)[1]<<"
      "<<(v0-u1 * t)[2]<<endl; cout<<"v1 after modification: "<<(v1-u2 *
      t)[0]<<" "<<(v1-u2 * t)[1]<<" "<<(v1-u2 * t)[2]<<endl;
      */
    }
    ...
    if(curLength < 0.95 * mOriLength)
    {
        ...
        T t3 = dot(vec1,vec2);
        T t1 = ((-2.0) * t3 + sqrt(4.0 * t3 * t3 - 4.0 * dot(vec2,vec2) *
    (dot(vec1,vec1) - 0.95 * 0.95 * mOriLength * mOriLength))) / (2.0 *
    dot(vec2,vec2)); T t2 = ((-2.0) * t3 - sqrt(4.0 * t3 * t3 - 4.0 *
    dot(vec2,vec2) * (dot(vec1,vec1) - 0.95 * 0.95 * mOriLength * mOriLength)))
    / (2.0 * dot(vec2,vec2)); if(t2 > 0) t = t2; else t = t1;
        mVerteces[0]->SetVel(v0 - u1 * t,0);
        mVerteces[1]->SetVel(v1 - u2 * t,0);
        cout<<"v0: "<<v0[0]<<" "<<v0[1]<<" "<<v0[2]<<endl;
        cout<<"v1: "<<v1[0]<<" "<<v1[1]<<" "<<v1[2]<<endl;
        ...
        cout<<"v0 after modification: "<<(v0-u1 * t)[0]<<" "<<(v0-u1 * t)[1]<<"
    "<<(v0-u1 * t)[2]<<endl; cout<<"v1 after modification: "<<(v1-u2 * t)[0]<<"
    "<<(v1-u2 * t)[1]<<" "<<(v1-u2 * t)[2]<<endl;

    }
    */
  }
  dtkT3<double> stiffForce =
      dir * ((mOriLength - curLength) * mStiffness / mOriLength);
  dtkT3<double> dampForce =
      -dir *
      (dot((v1 - v0), dir) * (timeslice * mStiffness / mOriLength + mDamp));
