...
/**
 * @file dtkPhysTetraMassSpring.h
 * @brief  dtkPhysTetraMassSpring 头文件
...
public:
  ...
  static Ptr New(bool fullUseAltSpring = false, double defaultMass = 2,
                 double defaultK = 0, double defaultDamp = 0,
                 double defaultPointDamp = 1.0,
                 double defaultPointResistence = 2.5,
                 dtkDouble3 defaultGravityAccel = dtkDouble3(0, 0, 0)) {
    return Ptr(new dtkPhysTetraMassSpring(
        fullUseAltSpring, defaultMass, defaultK, defaultDamp, defaultPointDamp,
        defaultPointResistence, defaultGravityAccel));
  }
  ...
  bool PreUpdate(double timeslice, ItrMethod method = Euler,
                 dtkID iteration = 0);
...
protected:
  dtkPhysTetraMassSpring(bool fullUseAltSpring = false,
                         double defaultMass = 2.0, double defaultK = 0.0,
                         double defaultDamp = 0.0,
                         double defaultPointDamp = 1.0,
                         double defaultPointResistence = 2.5,
                         dtkDouble3 defaultGravityAccel = dtkDouble3(0, 0, 0));
...
protected:
  ...
  std::vector<std::vector<double>>
      mOriLengths; // numberOfTetra * 7  每个四面体的高及对边的距离
