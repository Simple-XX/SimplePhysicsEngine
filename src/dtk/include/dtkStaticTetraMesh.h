...
/**
 * @file dtkStaticTetraMesh.h
 * @brief  dtkStaticTetraMesh 头文件
 ...
 * @brief 四面体网格
 * @author <>
 * @note This class is implemented following "Compact Array-Based Mesh Data
 * Structures"
 */
...
public:
  ...
  dtkID InsertTetra(dtkID v0, dtkID v1, dtkID v2, dtkID v3,
                    bool allowDegenated = false);
...
public:
  ...
  static dtkID3 GetAHFVertex(const dtkID &ahf, const std::vector<dtkID4> &ec,
  ...
  dtkID3 GetAHFVertex(const dtkID &ahf) { return GetAHFVertex(ahf, mEC, mB2F); }

  dtkID2 GetEdgeVertex(dtkID ahf, dtkStaticTetraMesh::Ptr mesh) {
    return GetEdgeVertex(ahf, mEC, mF2F, mB2F);
  }

  static dtkID2 GetEdgeVertex(dtkID ahf, const std::vector<dtkID4> &ec,
    ...
    return dtkID2(ec[c][EA2V[i - 1][j]], ec[c][EA2V[i - 1][(j + 1) % 3]]);
  }

  // Get the twin of selected AHF.
  // The twined AHF will anchored at the same vertex.
  static dtkID GetTwinAHF(const dtkID &ahf, const std::vector<dtkID4> &f2f,
                          const std::vector<dtkID> &b2f) {
    dtkID c, i, j;
    DecodeAHF(ahf, c, i, j);
  ...
  dtkID GetTwinAHF(const dtkID &ahf) { return GetTwinAHF(ahf, mF2F, mB2F); }

  // Get the HF <c, i> of ahf = AHF<c, i, j>.
  // The anchor information will lost.
  static dtkID GetHF(const dtkID &ahf) { return (ahf & 0xFFFFFFFC); }
  ...
  static void GetTwinAHFs(dtkID &ahf1, dtkID &ahf2, dtkID cell1, dtkID idx1,
    ...
    for (int i = 0; i < 3; ++i) {
      if (ec[cell1][EA2V[idx1 - 1][i]] == pt2)
        anchor2 = i;
      if (ec[cell2][EA2V[idx2 - 1][i]] == pt1)
        anchor1 = i;
    }
