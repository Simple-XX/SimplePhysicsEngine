...
/**
 * @file dtkPhysParticleSystem.h
 * @brief  dtkPhysParticleSystem 头文件
...
public:
  ...
  static Ptr New(double particleRadius, double particleMass,
                 double particleLifetime) {
    return Ptr(new dtkPhysParticleSystem(particleRadius, particleMass,
                                         particleLifetime));
  }
...
public:
  ...
  void SetPoint(dtkID id, const GK::Point3 &point) {
    mParticles[id]->SetPoint(point);
  }

  size_t GetNumberOfParticles() { return mParticles.size(); }
  const std::vector<dtkPhysParticle *> &GetParticles() const {
    return mParticles;
  }

  dtkID AddParticle(const GK::Point3 &position, const double &lifetime,
  ...
  void SetEmitPosition(const GK::Point3 &emitPosition) {
...
private:
  dtkPhysParticleSystem(double particleRadius, double particleMass,
                        double particleLifetime);
