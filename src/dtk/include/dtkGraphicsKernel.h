...
/**
 * @file dtkGraphicsKernel.h
 * @brief  dtkGraphicsKernel 头文件
 ...
 * 区间， 包括上限下限以及区间包含等操作
 */
template <typename Float> class dtkInterval {
public:
  /*
  ...
  */
  ...
  Float &operator[](const int &n) {
    return const_cast<Float &>(static_cast<const dtkInterval &>(*this)[n]);
  }
...
inline std::ostream &operator<<(std::ostream &stream,
                                const dtkDiscreteOrientationPolytope &kdop) {
  stream << "KDOP{ ";
...
public:
  ...
  template <class T> static bool Assign(T &tgt, const Type::Object &obj) {
    return CGAL::assign(tgt, obj);
  }

  static Type::Float SquaredDistance(const Type::Point3 &pt,
                                     const Type::Plane3 &plane);
  static Type::Float SquaredDistance(const Type::Plane3 &plane,
                                     const Type::Point3 &pt);
  static Type::Float SquaredDistance(const Type::Point3 &pt,
                                     const Type::Line3 &line);
  static Type::Float SquaredDistance(const Type::Line3 &line,
                                     const Type::Point3 &pt);

  static Type::Float SquaredArea(const Type::Point3 &p0, const Type::Point3 &p1,
                                 const Type::Point3 &p2);

  static Type::Vector3 CrossProduct(const Type::Vector3 &lhs,
                                    const Type::Vector3 &rhs);
  static Type::Float DotProduct(const Type::Vector3 &lhs,
                                const Type::Vector3 &rhs);
  static Type::Float DotProduct(const Type::Vector2 &lhs,
                                const Type::Vector2 &rhs);
  ...
  static Type::Point3 Centroid(const Type::Point3 &p0, const Type::Point3 &p1,
                               const Type::Point3 &p2);
  static Type::Point3 Centroid(const Type::Point3 &p0, const Type::Point3 &p1,
                               const Type::Point3 &p2, const Type::Point3 &p3);
  ...
  // Barycenter Weight
  static dtkDouble3 BarycentricWeight(const Type::Point3 &p0,
...
public:
  ...
  static dtkSign Orient2D(const Type::Point2 &a, const Type::Point2 &b,
                          const Type::Point2 &p);
...
public:
  static dtkSign BoundedSide(const Type::Tetrahedron3 &tetra,
                             const Type::Point3 &pt);
  static dtkSign BoundedSide(const Type::Sphere3 &sphere,
                             const Type::Point3 &pt);

public:
  static Type::Interval Merge(const Type::Interval &int_1,
                              const Type::Interval &int_2);
  static Type::KDOP Merge(const Type::KDOP &kdop_1, const Type::KDOP &kdop_2);

  static void Merge(Type::Interval &int_r, const Type::Interval &int_1,
                    const Type::Interval &int_2);
  static void Merge(Type::KDOP &kdop_r, const Type::KDOP &kdop_1,
                    const Type::KDOP &kdop_2);
};
