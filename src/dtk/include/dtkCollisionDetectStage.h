...
/**
 * @file dtkCollisionDetectStage.h
 * @brief  dtkCollisionDetectStage 头文件
...
public:
  ...
  typedef std::pair<dtkCollisionDetectHierarchy::Ptr,
                    dtkCollisionDetectHierarchy::Ptr>
      HierarchyPair;
...
public:
   ...
   * @note
   */
  void DoIntersect(HierarchyPair pair,
   ...
   * @note
   */
  void TraverseHierarchy(dtkCollisionDetectNode *node_1,
                         dtkCollisionDetectNode *node_2,
                         std::vector<IntersectResult::Ptr> &intersectResults,
                         bool self = false, bool ignore_extend = false);
  ...
  inline size_t GetNumberOfHierarchies() const { return mHierarchies.size(); }
  ...
  inline void ClearIntersectResult() { mIntersectResults.clear(); }
...
private:
  ...
  // 可能相交的层对.
  std::vector<HierarchyPair>
      mPossibleIntersectPairs; /**< only for temp-storage 可能相交的层对 */

  // 包围盒相交回调函数.
  static void BoxIntersectCallBack(const Box &a,
                                   const Box &b); /**< 包围盒相交回调函数 */

  // 可能相交的层的ID对.
  static std::vector<std::pair<size_t, size_t>>
      mPossibleIntersectPairIDs; /**< only for temp-storage. 可能相交的层的ID对.
                                  */
...
inline std::ostream &operator<<(std::ostream &stream,
                                const dtkCollisionDetectStage::Box &box) {
