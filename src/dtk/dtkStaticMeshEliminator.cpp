...
/**
 * @file dtkStaticMeshEliminator.cpp
 * @brief dtkStaticMeshEliminator 实现
...
void dtkStaticMeshEliminator::EliminateTriangles(
    dtkID id, size_t originalFaceNum, std::vector<dtkID3> &collisionFaces,
    bool singleMethod) {
  ...
  if (singleMethod) {
      ...
      for (dtkID j = 0; j < tetramesh->GetECTable().size(); j++) {
        if (Contains(tetramesh->GetECTable()[j], collisionFaces[i]) == true) {
          ...
          for (dtkID k = 0; k < trimesh->GetECTable().size(); k++) {
            if (IsIdentical(dtkID3(deleteTetra.a, deleteTetra.c, deleteTetra.b),
                            trimesh->GetECTable()[k]) == true) {
              findResult[0] = true;
              ...
              k--;
                           ...
                           dtkID3(deleteTetra.a, deleteTetra.b, deleteTetra.d),
                           trimesh->GetECTable()[k]) == true) {
              findResult[1] = true;
              surfaceDeleteFace.push_back(trimesh->GetECTable()[k]);
              trimesh->RemoveTriangle(trimesh->GetECTable()[k]);
              ...
              k--;
                           ...
                           dtkID3(deleteTetra.a, deleteTetra.d, deleteTetra.c),
                           trimesh->GetECTable()[k]) == true) {
              findResult[2] = true;
              surfaceDeleteFace.push_back(trimesh->GetECTable()[k]);
              trimesh->RemoveTriangle(trimesh->GetECTable()[k]);
              ...
              k--;
                           ...
                           dtkID3(deleteTetra.b, deleteTetra.c, deleteTetra.d),
                           trimesh->GetECTable()[k]) == true) {
              findResult[3] = true;
              surfaceDeleteFace.push_back(trimesh->GetECTable()[k]);
              trimesh->RemoveTriangle(trimesh->GetECTable()[k]);
          ...
          if (findResult[0] == false) {
            trimesh->InsertTriangle(deleteTetra.a, deleteTetra.b,
                                    deleteTetra.c);
            surfaceAddFace.push_back(
                dtkID3(deleteTetra.a, deleteTetra.b, deleteTetra.c));
          }
          if (findResult[1] == false) {
            trimesh->InsertTriangle(deleteTetra.a, deleteTetra.d,
                                    deleteTetra.b);
            surfaceAddFace.push_back(
                dtkID3(deleteTetra.a, deleteTetra.d, deleteTetra.b));
          }
          if (findResult[2] == false) {
            trimesh->InsertTriangle(deleteTetra.a, deleteTetra.c,
                                    deleteTetra.d);
            surfaceAddFace.push_back(
                dtkID3(deleteTetra.a, deleteTetra.c, deleteTetra.d));
          }
          if (findResult[3] == false) {
            trimesh->InsertTriangle(deleteTetra.b, deleteTetra.d,
                                    deleteTetra.c);
            surfaceAddFace.push_back(
                dtkID3(deleteTetra.b, deleteTetra.d, deleteTetra.c));
          }
          tetraMassSpring->DeleteTetra(j);
          tetramesh->RemoveTetra(tetramesh->GetECTable()[j]);
          // if ( mMeshEliminatorDeleteTetraCallback.find(id) !=
          // mMeshEliminatorDeleteTetraCallback.end() )
          //{
          //     ( mMeshEliminatorDeleteTetraCallback[id] )( id, j );
          // }
          dtkPoints::Ptr pts = trimesh->GetPoints();
          ChangeHierarchy(hierarchy, pts, collisionFaces[i], surfaceAddFace,
                          surfaceDeleteFace);
    ...
    for (dtkID i = 0; i < collisionFaces.size(); i++) {
      ...
      for (dtkID j = 0; j < 3; j++) {
        ...
        for (dtkID k = 0; k < tetramesh->GetECTable().size(); k++) {
          ...
          if (tetramesh->GetECTable()[k].a == collisionFaces[i][j] ||
            ...
            bool findTriangle[4] = {
                false, false, false,
                false}; // 记录四面体上哪些面被找到了，哪些没有被找到.
            dtkID4 deleteTetra = tetramesh->GetECTable()[k];
            for (dtkID m = 0; m < trimesh->GetECTable().size(); m++) {
              // 如果找到，则删除.
              if (IsIdentical(
                      dtkID3(deleteTetra.a, deleteTetra.c, deleteTetra.b),
                      trimesh->GetECTable()[m]) == true) {
                findTriangle[0] = true;
                if (m < originalFaceNum)
                  originalFaceNum--;
                surfaceDeleteFaceI.push_back(
                    dtkID3(deleteTetra.a, deleteTetra.c, deleteTetra.b));
                if (m < faceNum)
                  faceNum--;
                trimesh->RemoveTriangle(trimesh->GetECTable()[m]);
                m--;
              } else if (IsIdentical(dtkID3(deleteTetra.a, deleteTetra.b,
                                            deleteTetra.d),
                                     trimesh->GetECTable()[m]) == true) {
                findTriangle[1] = true;
                if (m < originalFaceNum)
                ...
                trimesh->RemoveTriangle(trimesh->GetECTable()[m]);
                m--;
              } else if (IsIdentical(dtkID3(deleteTetra.a, deleteTetra.d,
                                            deleteTetra.c),
                                     trimesh->GetECTable()[m]) == true) {
                findTriangle[2] = true;
                if (m < originalFaceNum)
                ...
                trimesh->RemoveTriangle(trimesh->GetECTable()[m]);
                m--;
              } else if (IsIdentical(dtkID3(deleteTetra.b, deleteTetra.c,
                                            deleteTetra.d),
                                     trimesh->GetECTable()[m]) == true) {
                findTriangle[3] = true;
                if (m < originalFaceNum)
            ...
            if (findTriangle[0] == false) {
              trimesh->InsertTriangle(
                  dtkID3(deleteTetra.a, deleteTetra.b, deleteTetra.c));
              surfaceAddFaceI.push_back(
                  dtkID3(deleteTetra.a, deleteTetra.b, deleteTetra.c));
            }
      ...
      // 改变碰撞检测树
      ChangeHierarchy(hierarchy, trimesh->GetPoints(), collisionFaces[i],
                      surfaceAddFaceI, surfaceDeleteFaceI);
    }
  }
  ...
  // 回调函数
  if (mMeshEliminatorResultsCallback.find(id) !=
          mMeshEliminatorResultsCallback.end() &&
      mMeshEliminatorResultsCallback[id] != 0) {
    (mMeshEliminatorResultsCallback[id])(
        id, originalFaceNum, surfaceAddFace,
        mMeshEliminatorResultsCallbackContext[id]);
  }
}

void dtkStaticMeshEliminator::ChangeHierarchy(
    dtkCollisionDetectHierarchy::Ptr hierarchy, dtkPoints::Ptr pts,
    dtkID3 collisionFace, const std::vector<dtkID3> &addFace,
  ...
  for (dtkID i = 0; i < hierarchy->GetNumberOfNodes(); i++) {
    ...
    for (dtkID j = 0; j < nodeTemp->GetNumOfPrimitives(); j++) {
      ...
      if (nodeTemp->IsLeaf() &&
          IsIdentical(collisionFace,
                      dtkID3(priDelete->mIDs[0], priDelete->mIDs[1],
                             priDelete->mIDs[2])) == true) {
...
void dtkStaticMeshEliminator::GroupModel(
    dtkCollisionDetectHierarchy::Ptr hierarchy) {}
