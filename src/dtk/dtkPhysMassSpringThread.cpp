...
/**
 * @file dtkPhysMassSpringThread.cpp
 * @brief dtkPhysMassSpringThread 实现
...
dtkPhysMassSpringThread::dtkPhysMassSpringThread(
    double interval, int length, dtkT3<double> firstPos, Orientation ori,
    double mass, double edgeStiff, double bendStiff, double torsionStiff,
...
void dtkPhysMassSpringThread::constructThreadMesh() {
  ...
  for (dtkID i = 0; i < mLength * 2 + 1; i++) {
    ...
    if (i % 2 == 0) {
      ...
      tempPoint = mFirstPos + oriY * tempRadius * cos(angle) +
                  oriZ * tempRadius * sin(angle) +
                  oriX * (mInterval * (float)((i - 1) / 2) + mInterval / 2.0);

      mPointsPtr->SetPoint(i,
                           GK::Point3(tempPoint.x, tempPoint.y, tempPoint.z));
    }
  }
...
void dtkPhysMassSpringThread::ImpulsePropagate(dtkT3<double> impulse,
                                               dtkID pointID, dtkID opt,
                                               size_t range) {
  dtkDouble3 tempPos1, tempPos2, e, tempImpulse1, tempImpulse2,
      tempImpulseBuffer1, tempImpulseBuffer2;
  tempImpulseBuffer1 = tempImpulseBuffer2 = tempImpulse1 = tempImpulse2 =
      impulse;
  for (dtkID i = 0; i < range; i++) {
    if (opt != 1 && (pointID + (i + 1) * 2) <= mLength * 2 && (i + 1) < range) {
      ...
      this->GetMassPoint(pointID + i * 2 + 1)
          ->AddImpulse((tempImpulse1 + tempImpulseBuffer1) * 0.5);
    ...
    if (opt != 2 && pointID >= (i + 1) * 2 && (i + 1) < range) {
      ...
      this->GetMassPoint(pointID - i * 2 - 1)
          ->AddImpulse((tempImpulse2 + tempImpulseBuffer2) * 0.5);
