...
/**
 * @file dtkIntersectTest.cpp
 * @brief dtkIntersectTest 实现
...
bool dtkIntersectTest::Test(GK::Point3 &pt, const GK::Segment3 &seg,
                            const GK::Triangle3 &tri) {
...
bool dtkIntersectTest::Test(GK::Point3 &pt, const GK::Line3 &line,
                            const GK::Triangle3 &tri) {
  GK::Plane3 plane(tri.vertex(0), tri.vertex(1), tri.vertex(2));
...
bool dtkIntersectTest::DoIntersect(const GK::Tetrahedron3 &tetra,
  ...
  if (!IsOverlap(amin0, amax0, amin1, amax1))
    return false;
...
bool dtkIntersectTest::DoIntersect(const GK::Triangle2 &triangle,
  ...
  tpn = CGAL::Aff_transformation_2<GK::K>(CGAL::TRANSLATION,
                                          GK::Vector2(v0.x(), v0.y()));
  tp = CGAL::Aff_transformation_2<GK::K>(CGAL::TRANSLATION,
                                         GK::Vector2(-v0.x(), -v0.y()));
  trans = tpn * (rotate * tp);
...
bool dtkIntersectTest::DoIntersect(const GK::Triangle3 &triangle,
                                   const GK::Sphere3 &sphere) {
...
void dtkIntersectTest::ProjectToAxis(GK::Float &amin, GK::Float &amax,
...
bool dtkIntersectTest::IsOverlap(GK::Float min0, GK::Float max0, GK::Float min1,
                                 GK::Float max1) {
  return !(max1 < min0 || min1 > max0);
}

bool dtkIntersectTest::DoDistanceIntersect(const GK::Triangle3 &tri,
  ...
  //  depth_1, depth_2: present the distance of triangle face with segment's two
  //  point.
  double depth_1 =
      distance - GK::DotProduct(GK::Vector3(triP1, segP1), tri_unit_normal);
  double depth_2 =
      distance - GK::DotProduct(GK::Vector3(triP1, segP2), tri_unit_normal);
  ...
  if (!intersected) {
    ...
    if (uvw_segP1[0] >= 0 && uvw_segP1[0] <= 1 && uvw_segP1[1] >= 0 &&
        uvw_segP1[1] <= 1 && uvw_segP1[2] >= 0 && uvw_segP1[2] <= 1 &&
        depth_1 >= 0 && depth_1 <= distance * 2.0) {
      intersected = true;
#ifdef DTK_INTERSECTTEST_DEBUG
      cout << "intersect inside" << endl;
#endif
    } else if (uvw_segP2[0] >= 0 && uvw_segP2[0] <= 1 && uvw_segP2[1] >= 0 &&
               uvw_segP2[1] <= 1 && uvw_segP2[2] >= 0 && uvw_segP2[2] <= 1 &&
  ...
  if (intersected) {
    // depth_1 <= 0 : present no touching of the point of segment and triangle
    // which has stickness. depth_1 > 0:  present touching.
    ...
    // CGAL function: according to two points and correspond two weight to
    // compute barycenter. GK::BarycentricWeight: at first project segP1 to the
    // bottom triangle, then, compute the weight.
    GK::Point3 P2 = barycenter(segP1, uv_seg[0], segP2, uv_seg[1]);
    dtkDouble3 uvw_tri = GK::BarycentricWeight(P2, triP1, triP2, triP3);
    // GK::Point3 P1 = barycenter( triP1, uvw_tri[0], triP2, uvw_tri[1], triP3,
    // uvw_tri[2] );
    for (int i = 0; i < 3; i++) {
      int a = i;
      int b = (i + 1) % 3;
    ...
    if (DoDistanceIntersect(GK::Segment3(triP1, triP2), seg, distance,
                            result)) {
#ifdef DTK_INTERSECTTEST_DEBUG
      ...
      intersected = true;
    } else if (DoDistanceIntersect(GK::Segment3(triP2, triP3), seg, distance,
                                   result)) {
#ifdef DTK_INTERSECTTEST_DEBUG
      cout << "intersect with side line" << endl;
#endif
      ...
      intersected = true;
    } else if (DoDistanceIntersect(GK::Segment3(triP3, triP1), seg, distance,
                                   result)) {
#ifdef DTK_INTERSECTTEST_DEBUG
      cout << "intersect with side line" << endl;
#endif
...
bool dtkIntersectTest::DoDistanceIntersect(const GK::Segment3 &seg_0,
  ...
  if (CGAL::collinear(p11, p12, p12 + v_edge_2)) {
    ...
    GK::Vector3 normal = GK::Normalize(GK::CrossProduct(
        GK::CrossProduct(v_side_edge_11_21, v_edge_1), -v_edge_1));
    ...
    if (length < distance) {
      ...
      double fraction_1 = GK::DotProduct(n_edge_face_2, v_side_edge_11_21) /
                          GK::DotProduct(n_edge_face_2, v_edge_1);
      double fraction_2 = -GK::DotProduct(n_edge_face_1, v_side_edge_11_21) /
                          GK::DotProduct(n_edge_face_1, v_edge_2);
    ...
    if (intersect) {
      ...
      assert(length >= 0);
      result = IntersectResult::New();
      result->SetProperty(INTERSECT_NORMAL,
                          apply_normal * (distance - length) * 2.0);
      result->SetProperty(INTERSECT_WEIGHT_1, uv1);
      result->SetProperty(INTERSECT_WEIGHT_2, uv2);
    }
  }
  ...
  if (!intersect) {
    double penetrate[4] = {dtkDoubleMin, dtkDoubleMin, dtkDoubleMin,
                           dtkDoubleMin};
    if (length < distance) {
    ...
    if (intersect) // parallel line end intersect
    {
      ...
      assert(length >= 0);
      result->SetProperty(INTERSECT_NORMAL,
                          apply_normal * (distance * 2.0 - length));
      result->SetProperty(INTERSECT_WEIGHT_1, uv1);
      result->SetProperty(INTERSECT_WEIGHT_2, uv2);
    }
  }
  ...
  && abs(GK::DotProduct(v22,-v_edge_1_normal) / edge_2_length) <= 1)
              ...
              {
                  if(abs(GK::DotProduct(v11, v_edge_2_normal) / edge_1_length)
  <= 1 && abs(GK::DotProduct(v21, v_edge_2_normal) / edge_1_length) <= 1)
                      intersect = true;
                  else
                  {
                      if(abs(GK::DotProduct(v12, v_edge_2_normal) /
  edge_1_length) <= 1 && abs(GK::DotProduct(v22, v_edge_2_normal) /
  edge_1_length) <= 1) intersect = true;
                  }
              }
          }
      }
      ...
      if( intersect )
      {
          ...
          normal = normal * ( distance - abs(length) ) * 2; // double penetrate
  depth result->SetProperty( INTERSECT_NORMAL, normal ); result->SetProperty(
  INTERSECT_WEIGHT_1, uv1 ); result->SetProperty( INTERSECT_WEIGHT_2, uv2 );
      }
      return intersect;
  }
  ...
  double fraction_1 = GK::DotProduct ( n_edge_face_2, v_side_edge ) /
  GK::DotProduct( n_edge_face_2, v_edge_1);
  //GK::Vector2 uv_1( 1.0 - fraction_1, fraction_1 );
  double fraction_2 = - GK::DotProduct( n_edge_face_1, v_side_edge ) /
  GK::DotProduct( n_edge_face_1, v_edge_2);
...
bool dtkIntersectTest::DoDistanceIntersect(const GK::Triangle3 &tri_1,
  ...
  if (invert == 2) // tri_2 contains tri_1
  {
    ...
    for (dtkID i = 0; i < 3; i++) {
      ...
      dtkDouble3 uvw =
          GK::BarycentricWeight(tri_1[i], tri_2[0], tri_2[1], tri_2[2]);
      if (uvw[0] < 0 || uvw[0] > 1 || uvw[1] < 0 || uvw[2] > 1 || uvw[2] < 0 ||
          uvw[2] > 1)
        continue;
...
bool dtkIntersectTest::DoDistanceIntersect(const GK::Triangle3 &tri,
  ...
  dtkDouble3 uvw =
      GK::BarycentricWeight(sphere.center(), tri[0], tri[1], tri[2]);
  if (uvw[0] < 0 || uvw[0] > 1 || uvw[1] < 0 || uvw[2] > 1 || uvw[2] < 0 ||
      uvw[2] > 1)
    return false;
...
bool dtkIntersectTest::DoIntersect(const GK::Triangle3 &triangle_1,
...
bool dtkIntersectTest::DoIntersect(const GK::Segment3 &seg_1,
...
bool dtkIntersectTest::Test(const GK::BBox3 &box1, const GK::BBox3 &box2) {
...
bool dtkIntersectTest::DoIntersect(const GK::BBox3 &box1,
                                   const GK::BBox3 &box2) {
  return CGAL::do_overlap(box1, box2);
}

bool dtkIntersectTest::DoIntersect(const GK::Segment2 &seg0,
                                   const GK::Segment2 &seg1) {
  return CGAL::do_intersect(seg0, seg1);
}

bool dtkIntersectTest::DoIntersect(const GK::Line3 &line,
                                   const GK::Triangle3 &tri) {
...
bool dtkIntersectTest::DoIntersect(const GK::Segment3 &seg,
                                   const GK::Triangle3 &tri) {
...
bool dtkIntersectTest::DoIntersect(const GK::Ray3 &ray,
                                   const GK::Plane3 &plane) {
...
bool dtkIntersectTest::DoIntersect(const GK::Interval &int_1,
...
bool dtkIntersectTest::DoIntersect(const GK::KDOP &kdop_1,
...
void dtkIntersectTest::UpdateMinMax(GK::Float &tmin, GK::Float &tmax,
                                    const GK::Float &val) {
  if (val > tmax)
    tmax = val;
  if (val < tmin)
